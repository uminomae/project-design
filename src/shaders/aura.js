  ...
  material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        a += (1.0 / (r + 0.2)) * 0.8 * sin(u_time * 0.5);
        vec2 p_vortex = vec2(cos(a), sin(a)) * r;
        p = mix(p, p_vortex, w4);
        ...
        float t = u_time * (0.2 + s * 0.3);

        vec2 q = vec2(fbm(p + vec2(0.0, t * 0.4)), fbm(p + vec2(t * 0.5, 0.0)));
        vec2 r_vec = vec2(fbm(p + 2.5 * q + vec2(1.7, 9.2) + 0.15 * t),
                          fbm(p + 2.5 * q + vec2(8.3, 2.8) + 0.12 * t));

        float f = fbm(p + 3.0 * r_vec);

        vec3 whiteCol = vec3(1.0, 1.0, 1.0);
        vec3 inkCol = vec3(0.08, 0.12, 0.18);
        vec3 goldCol = vec3(0.85, 0.70, 0.35);
        vec3 cyanCol = vec3(0.25, 0.75, 0.95);

        float density = smoothstep(0.1, 0.85, f);
        vec3 color = mix(whiteCol, inkCol, density * 0.6);

        float cyanStr = length(q) * (0.4 + w3 * 0.4 + w5 * 0.2);
        float goldStr = length(r_vec) * (0.5 + w4 * 0.5 + w5 * 0.3);

        color = mix(color, cyanCol, cyanStr * density);
        color = mix(color, goldCol, goldStr * density);

        float wave = sin(length(p_base - q) * 18.0 - u_time * 2.5) * 0.5 + 0.5;
        color = mix(color, whiteCol, wave * 0.15 * (w2 + w3*0.5));

        float alpha = density * (0.65 + w5 * 0.2) + length(q) * 0.2 + length(r_vec) * 0.15;
        float vignette = 1.0 - length(uv - 0.5) * 1.2;
        alpha *= smoothstep(0.0, 0.5, vignette);
