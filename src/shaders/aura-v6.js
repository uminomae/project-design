// src/shaders/aura-v6.js — Light network (caustics) + kaleidoscopic fold + black hole
...
function init() {
  ...
  material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      float lightNetwork(vec2 p, float t) {
          vec2 q = p;
          float c = 0.0;
          for (int i = 0; i < 4; i++) {
              q = p + vec2(cos(t + q.y * 2.0), sin(t + q.x * 2.0)) * 0.5;
              c += 0.05 / (length(q) + 0.01);
              p = q * 1.6;
              p *= rot(t * 0.1);
          }
          return c;
      ...
      void main() {
        ...
        float dist = length(p);
        p /= (1.0 + dist * dist * mix(2.0, 15.0, s));
        ...
        float networkIntensity = lightNetwork(p + q, t_rhythmic * 1.5) * mix(0.2, 1.0, w4 + w5);

        float eventHorizonRadius = mix(0.1, 0.2, w5);
        ...
        float diskAura = field * field * lines + field * field * field * 0.3 + networkIntensity * 0.5;
        ...
        vec3 color = mix(deepBlue, brightCyan, clamp(diskAura * 3.0, 0.0, 1.0));
        color = mix(color, pinkish, clamp(length(q), 0.0, 1.0) * 0.3);

        vec3 networkColor = mix(goldCol, vec3(1.0), networkIntensity);
        color = mix(color, networkColor, clamp(networkIntensity * 1.2, 0.0, 1.0));
        ...
        float brightness = length(field) + networkIntensity * 0.5;
        float finalAlpha = diskMask * (brightness * 0.7 + lines * 0.4 + networkIntensity * 0.6);
