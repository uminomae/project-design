...
> **高校生向けのやさしい解説**
>
> このページは kesson-space という別のリポジトリの「目次」のようなページです。kesson-space は、「欠損（何かが足りない感覚）を出発点にして考える」という理論を、精神分析・愛着理論・臨床心理学などの知見と結びつけて深めている場所。この wiki にある主な概念や人物のページは、もとをたどるとここに行き着きます。

kesson-space (ks) は[[欠損駆動思考]]の理論的基盤と実践表現を探索するリポジトリ。コア概念群、概念ノート、臨床心理学テキスト、表現集を正本として保持する。この wiki の多くのページが ks をソースとしている。
...
| 概念 | wiki ページ | 概要 |
|------|------------|------|
| [[欠損駆動思考]] | concepts/欠損駆動思考 | 棄却される誤差を問いとして拾う態度 |
| [[欠損]] | concepts/欠損 | 予想と現実の誤差を「欠け」として捉えた主観的経験 |
| [[抱持]] | concepts/抱持 | 反射的に処理せず誤差を問いとして保持する機能 |
| [[情動の構成]] | concepts/情動の構成 | 欠損が生存-信頼軸で評価され情動として構成されるプロセス |
...
- [[アウェアネスモデル]] -- 意識の構造モデル（as + ks 共同ソース）
...
as は ks の理論を意識研究の文脈で検証・拡張するリポジトリ。[[アウェアネスモデル]] と [[内受容感覚]] は ks と as の両方をソースとする。as の概要は [[cross-refs/awareness-space-overview|awareness-space 概要]] を参照。
