...
プロジェクトデザイン論は「やること（Doing）」と「起きていること（Being）」を含む出来事としてのプロジェクトを探索する。免疫系の多様性生成は「あらかじめ全解を持たずに状況に応じて解を生成する」システムの典型例である。プロジェクトも同様に、全ての状況を事前に規定することなく、文脈（抗原）に応答して適切な応答（抗体）を体細胞的に生成する動的プロセスである。固定された計画ではなく、遭遇するものに応じてセグメントを組み合わせ新たな対応を生み出す構造は、PD の「文脈応答型の生成」という観点と共鳴する。
