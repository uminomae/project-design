...
プロジェクトデザイン論において「確実性の探求」は逆説的な意味を持つ。プロジェクトは不確実性の中で行為する実践であり、デューイが批判した「不変の計画への固執」は PD の文脈では機能しない。「確実性」から「保証された行為の賢明さ」への転換は、完璧な事前計画よりも探究的実践を重視する PD の姿勢と一致する。また行為・制作を知識と同等に位置づける視点は「Doing（やること）」を Being と等価に扱う PD の基本的立場と共鳴する。プロジェクトは世界を操作するだけでなく、操作を通じて知識を生成するプロセスでもある。
