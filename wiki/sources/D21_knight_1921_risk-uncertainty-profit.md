...
PD（プロジェクトデザイン論）において Knight の不確実性論は、「プロジェクトは本質的に真の不確実性のもとで行われる」という命題を支持する。プロジェクトが扱う問題の多くは、事前に確率分布を設定できない「起きていること（Being）」の領域にある。この文脈で Knight の起業家的判断は、PD における「計画の外側で決断する」営みの理論的基盤となる。また Knight の自由企業制度分析が「欠陥を正直に認めながら擁護する」姿勢を取ることは、PD が「プロジェクトの失敗・迷走・欠損を保持論点として扱う」態度と共鳴する。さらに「真の不確実性には計算ではなく判断が対応する」という主張は、Love駆動開発における「感情・関係・意図が判断を駆動する局面」を正当化する経済学的根拠として参照できる。
