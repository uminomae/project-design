...
ENSOモデルが示す「内部フィードバックだけで不規則変動が生まれる」という知見は、外部の攪乱がなくても複雑系が予測困難なふるまいを示すことの原理的な証拠である。PD において「プロジェクトの展開がランダムに見えても、内部の相互作用構造によって駆動されている」という視点を支持する。また、「熱蓄積と放電」というメカニズムは、プロジェクトにおけるエネルギーの蓄積・解放サイクル（緊張の高まりとそれを伴うイベント）の比喩として参照できる。大気・海洋という異なる時定数を持つサブシステムの結合が全体のリズムを規定するという構造は、プロジェクト内の異なる活動リズムの結合という問いと共鳴する。
