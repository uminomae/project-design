...
プロジェクトデザイン論における「Being（起きていること）」の次元は、プロジェクトが環境と相互作用しながら徐々に形を変えていくプロセスに関わる。植物遷移論は、初期状態から最終形態へ向かう発展プロセスを段階的・機能的に記述する方法論を提供する。プロジェクトもまた「裸地」（出発点）から始まり、環境からの影響と内部の反応が積み重なってクライマックス（目標状態）へ近づく。遷移が環境を能動的に改変する「反応」を含むことは、プロジェクトが状況を変えながら自らも変化するという相互構成的な動態と対応する。
