...
ICH Q8(R2)が定義する「デザインスペース」は、プロジェクトデザインにおける「設計の許容域」と構造的に対応する。品質は事後の検査で担保するのではなく、開発プロセスに織り込む（quality cannot be tested into products）という原則は、PD が「やること（Doing）」の設計段階で「起きていること（Being）」の可変域を意識的に設定するアプローチと重なる。また、継続的改善とライフサイクル管理の思想は、PD においてプロジェクトが静的な計画の実行ではなく、動的な学習と適応の過程であるという理解を補強する。
