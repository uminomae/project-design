...
本原典が示す自己免疫疾患の3フェーズ構造（開始→増幅→寛解）は、プロジェクトにおける状態変化のモデルとして参照できる。PDの観点では、「起きていること（Being）」として慢性的な不均衡状態が継続し、「やること（Doing）」としての介入が均衡を再建しようとする。しかし現状の治療戦略が発症の根本原因（開始フェーズ）に対処できていないことは、PDにおける「問いを適切に立てる」ことの重要性と対応する。症状の抑制と問題の根治は異なる次元の介入であり、プロジェクトがどのフェーズの問いを扱っているかを意識することがデザインの核心となる。
