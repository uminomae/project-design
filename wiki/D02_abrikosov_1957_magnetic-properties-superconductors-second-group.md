...
アブリコソフ渦糸格子の発見は、プロジェクトデザイン（PD）における「秩序の発生」という問いと接続する。外部からの圧力（磁場）が一定の閾値を超えると、内部に規則的なパターン（渦糸格子）が自発的に形成される。これは Being（起きていること）としてのプロジェクト現象——外部環境の変化が内部の自己組織化を引き起こす——の物理的な類比として読める。

また、超伝導状態と正常状態が空間的に共存する「混合状態」は、プロジェクトにおける複数の状態が同時に存在する局面——たとえば変革期のプロジェクトで旧来の秩序と新しい秩序が局所的に共存する状態——の構造的アナロジーを提供する。第二種超伝導体が第一種よりもはるかに広い磁場範囲で超伝導性を維持できるように、複数状態の共存を許容する系は単純な二元対立より堅牢でありうる。
