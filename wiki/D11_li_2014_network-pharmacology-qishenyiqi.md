...
漢方処方のネットワーク薬理学は、プロジェクトデザイン論が「プロジェクト」に求める複合的・動的な把握とよく対応する。単一因果の線形モデルでは捉えられない複雑な系（多成分・多標的・多経路）を、ネットワーク構造として俯瞰するアプローチは、PD における「起きていること（Being）」の把握方法と類似している。プロジェクトもまた、単一の介入が直線的に結果に結びつくのではなく、複数の関係者・要因・文脈が絡み合う動的なネットワークとして存在している。この原典は、複雑系への介入において「ネットワーク全体の設計」という発想の重要性を傍証する。
