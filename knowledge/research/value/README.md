...
| *(予定)* value-integrated.md | 統合調査レポート（3レポートの強み統合 + 弱点補完） | Phase C（統合調査） |

## 読み方

1. **全体像を掴む** → `research.md`（調査設計と到達点）
2. **個別レポートを読む** → `value-chatgpt-*.md`, `value-chatgptpro-*.md`, `value-codex-*.md`
3. **比較結果を見る** → `value-llm-comparison.md`（9軸評価 + 使い分け推奨）
4. **調査過程を追う** → `PROCESS-LOG.md`（時系列の操作記録）
5. **統合成果を読む** → `value-integrated.md`（完成次第）
