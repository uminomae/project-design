...
- **Bikard 2020 の idea-twins はこの交絡を攻めるために作られた識別ツール**（同じアイデア＝内容を固定して「誰が担ったか」の効果を切り出す自然実験）。RR-032 の Shalizi–Thomas「共通原因 vs 影響」を社会実データで攻略する装置が実在する。

→ **RR-034 の◐仮説「合成の空白は識別問題を含む構造的難所」は、思弁でなく実在の研究フロンティアだった**。標準モデル（Poisson＝独立）を、2025 年の大規模データが long-tail で揺さぶり、識別ツール（idea twins）が既にある。
...
1. **同時発見の合成（層A）は、実は半世紀の定量研究がある**——ただし標準モデル(Poisson)は**独立を仮定**＝「同期」ではなく「共通原因での並行独立到達」を測ってきた。2025 年の 4000万論文が初めて Poisson を long-tail で棄却＝**独立からの逸脱**を大規模に示した。**"人間間の同期・干渉"に最も近い実データはここにあり、しかも交絡（共通原因 vs 影響）は未分解のまま研究フロンティアとして開いている**（idea-twins がその攻略ツール）。
...
**残る宿題**: ①Li ら 2025 の long-tail 逸脱を common-cause 不均一性 vs contagion に分解した研究の探索（＝Shalizi–Thomas 交絡の社会実データでの決着状況）。②Bikard idea-twins データセットで「同期・伝染」を測った後続研究の網羅。③Kleinberg 2002 burst detection を multiples 時系列に当てた研究の有無（RR-034 が名指しした候補・本探索では層Cの Lorenz-Spreen が近縁だが直接は未確認）。いずれも body ペイウォール多数＝優先度は READER 反映判断（pjdhiro）後に。
