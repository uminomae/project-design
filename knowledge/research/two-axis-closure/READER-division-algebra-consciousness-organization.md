...
> **この読み物の二つの声**——「私」（pjdhiro）の言葉は、**背景・体験・問い、そして回答への感想（しっくり感の判定）だけ**。それ以外の本文——**数学・物理の解説、文献調査、読みの候補、機械検証**——は、私の問いに対する **AI（Claude Fable 5）の回答**だ。役割分担は一貫している: **私が問い、Fable5 が候補を答え、私が採否を判定する**（判定の記録は RR-010）。数学の解説を、私が自分の言葉として語っているのではない。なお本文の「**機械検証済み**」は、Fable5 が該当の数学的主張をプログラムで数値的に確かめたことを指す——スクリプトは保存してあり、誰でも再実行できる。
...
1人を区別すると（外的な非対称：希少スキル等）、その1人を通る3本の線が、残る6人を「その人と組む3組のペア」に割ってしまう——**区別が、構造の分割を連れてくる**（§4 冒頭の「e₁ を掛けると3つの平面が同時に回る」と同じ配線の裏表。機械検証済み）。
