...
> 二つの素朴体験から出発し、数の梯子（ℝ→ℂ→ℍ→𝕆）をたどって、「なぜ 3 と 7 だけが特別なのか」を、定理・現実のたとえ・人間関係の三層で通す読み物。前提（解釈の賭け）と定理（硬い部分）は分けて記す。図は単体表示できる自己完結 SVG。
...
そして **7**：経路は7で21、8で28。単独の人間が全関係図を同時に保持できる限界（Miller 7±2 の水準）を、ちょうど 7↔8 で越える。赤い棒（28）が「もう一望できない」領域。
