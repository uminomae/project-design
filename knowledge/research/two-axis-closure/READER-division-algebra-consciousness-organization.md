...
> この読み物は、二つの素朴な体験と、「生存と信頼の二軸」という意識の仮説から出発します。そのうえで、経営でよく言われる「足し算ではなく、掛け算」という言い回しを、まじめに問い直します。その「掛け算」を**回転の掛け算**として受け取り、数の梯子（ℝ→ℂ→ℍ→𝕆）をたどって確かめていきます。中心の問いは「なぜ 3 と 7 だけが特別なのか」です。これを、①定理（数学で確かめられること）、②現実のたとえ、③人間関係でいうと、という三つの層で説明します。数学の定理（確かな部分）と、解釈の賭け（まだ確かめきれていない読み）は、きちんと分けて書きます。検証が進むたびに、§7「この調査でわかったこと」を書き足していきます——**うまくいかなかった部分も含めて**書きます。図は、それだけ取り出しても意味が通る自己完結の SVG です。
...
>
> **判定のことば**について、先に決めておきます。読みの候補につく状態のラベルは、次の5段階で統一します。**採用**（判定済みで、本文の骨格になっているもの）／**方向確定**（方向は定まったが、言葉の精度はまだ調整中のもの）／**読み候補**（Fable5 が提案し、まだ判定を待っているもの）／**探索中**（調査そのものが進行中のもの）／**棄却**（判定済みで、採らないと決めたもの。理由も残します）。あわせて、二つの言葉を先に定義します。「**保持論点**」とは、不快さや混乱ごと、**急いで答えを出さずに残す**と決めた問いのことです。「**しっくり感**」とは、私（pjdhiro）が読みを採るかどうかを決めるときの、体感的な基準のことです。論証ではなく、**体験と照らしあわせたときの納得**を指します。
...
この問いには、たどってきた道があります。私は「意識とは何か」を、**生存と信頼の二軸**という仮説で深掘りしてきました（姉妹探究 [意識とは — awareness-space](https://uminomae.github.io/awareness-space/)。神経現象学の知見とも衝突しない形で組んであります）。意識が二軸なら、意識の状態は**平面の上の点**として表せます。そして平面の上でいちばん自然な動きは、足し引きではなく**回転**です。次の §2 で見るとおり、平面の回転を担う数が複素数で、それを立体に広げた数が四元数です。
...
（図の中の「掛け算ができる／割り算ができる」は、§3・§4 で定義する意味の略記です。「掛け算ができる」は**答えが人の軸に着地する**こと、「割り算ができる」は**結果から巻き戻せる**ことを指します。「2人以下では掛け算そのものが計算できない」という意味ではありません。）

<details>
<summary><strong>📖 補足 — 「3人・7人」は、これまで何と言われてきたか（そして、この探究はどの角度か）</strong></summary>

チームの人数については、これまでたくさんの学説と経験則が語られてきました。まず、それを並べておきます（棚卸しは Fable5 の調査によります。詳細と出典は RR-023 にあります）。
...
> **私の問い**: 「足し算ではなく、掛け算」——その掛け算とは、どんな掛け算なのか。「回転の掛け算」のことだとしたら、何が見えるのか。（§1／原点セッション 2026-07-02）
>
> **ここから §4 までは、この問いへの Fable5 の回答です**。数学の説明・たとえ・読みの候補は Fable5 が作り、その読みを採るかどうかだけを私が判定しています。
...
<details>
<summary><strong>コラム（寄り道）— ブルーム橋の落書き</strong></summary>

四元数を見つけたのは、アイルランドの数学者ハミルトンです。伝わっている逸話を紹介します（歴史の細部は Fable5 が調べました。情景の部分は伝承が混ざっている可能性があります）。

1843年10月16日、ハミルトンは妻と散歩をしていました。何年も解けなかった「立体をなめらかに回す数」の掛け算の規則が、ダブリンのブルーム橋（ブルーハム橋）を渡るときに、突然ひらめいたと言われています。ハミルトンはうれしさのあまり、持っていたナイフで橋の石に、その規則を刻みました。

刻んだ式は `i² = j² = k² = ijk = −1` です。この短い一行が、四元数の掛け算のすべてを決めています。この読み物の §3 で出てくる `i·j = k` などの規則も、全部この一行から出てきます。

いまでは橋に記念のプレートがはめられていて、数学者たちが毎年その場所を訪ねる小さな巡礼になっています。ひらめきの瞬間を石に刻む、という珍しい記録です。（RR-014）
...
>
> 例外はひとつだけあります。「×0」は戻せません。6×0＝0、4×0＝0。結果のゼロを見ても、元が6だったか4だったか、もう誰にもわかりません。ふつうの数で、こういう「戻せない事故」が起きるのは、0を掛けたときだけです。だから普段は安心して掛け算ができて、必要なら割り算で戻れます。「割り算ができる世界」とは、この事故が構造的に起きないと保証されている世界のことです（この「割り算ができる」を、もっと細かく層に分けた説明を、§4 の「割り算ができる、を四つの層に分ける」で行います。零因子については §4 末尾のコラム「つむじと零因子」で説明します）。
...
> 📖 **人間関係の「掛け算」とは何か** — ここがこの読み物のいちばん深い賭けです。まだ言葉が完成していません（保持論点）。問いの形は、こうです。四元数の本質と、3人の関係の本質は、同じ形をしているのではないか。だとすれば、それはどんな関係のことか。いまの見当を、出発点の二軸に忠実なかたちで書いておきます（候補の言語化は Fable5、採否の判定は pjdhiro のしっくり感によります。記録は RR-010）。
>
...
> - メンバー一人ひとりは、それぞれ自分の軸を持っています。その軸の正体は「見方・視点」ではありません。場を取り巻くエネルギーです。揺らぎや振幅のような、もう一つの回転です。それぞれが同じ出来事を、違う向きに生存↔信頼変換して回しています（認知のスタイルの話に縮めないでください。§6 の「場は増えない。増えるのは、場を取り巻くエネルギー」と同じ線です）。
...
> - **足し算ではなく、掛け算**。足し算は順序を替えても同じです（3＋5 ＝ 5＋3）。頭数を寄せ集める世界です。回転の合成は、順序で向きが変わります。`i·j = k` ですが `j·i = −k` です。A の軸を先に通すか、B の軸を先に通すかで、生まれる回転の向きが逆になります。チームは頭数の足し算ではなく、順序と向きが意味を持つ掛け算でできています。§1 で立てた「その掛け算とは、どんな掛け算なのか」という問いは、ここに着地します。回転の掛け算です。これがこの読み物の賭けの核心です。
>
> **一つ、大事な限定をつけます**。ここでいう「一人が動くとあいだが回る」「順序で向きが変わる」は、その人が**どういう状態か**を言っているのではありません。二人が**関わる局面・観測する局面**で何が起きるか、を言っています（この区別は §7 の「地図と色眼鏡」につながります。人の状態そのものが順序で書き換わるという主張ではなく、関わり方・見方の順序が効く、という主張です）。
>
> **正直な注意を三つ、しておきます**。第一に、この「回し方の合成」という読みは解釈の賭けです。まだ検証の途中です（§7 結果1）。第二に、これを観測するときは、「どのペアの件も、着地する第三の軸（ホーム）が一意に決まっているか」という粗い代理指標に落とします（§7 結果3）。代理は、意味そのものではありません。第三に、「掛け算」という言葉からは「足し算以上にどんどん増える」を想像しやすいです。そして、その手応え自体は幻ではありません。ただし数学の帳簿では、回転の掛け算は各人の大きさを保ちます。跳ね上がりの正体は、波の重ね合わせ（足し算）の干渉です。位相が揃った瞬間の増幅で、回転はその揃い方を決めるダイヤルです（§8「よくある誤解」で、この感覚の根まで掘ります）。

**私の感想・判定**: 「一人が動くと、あいだが回る」——その言い方が、高校生にわかりやすい（採用）。回転という掛け算で、順序によって**向きが変わる**ことも重要（採用）。（2026-07-03・記録は RR-010 Q2/Q3）
...
四元数を使うと、この行きづまりが起きません。どの向きからどの向きへも、なめらかに回せます。だから今日でも、ゲームがキャラクターやカメラを回すとき、宇宙機やドローンが姿勢を制御するとき、四元数が中で働いています。§2 で「ゲームがキャラクターを回すのに今も使われている本物の道具」と書いたのは、このことです。（RR-014）
...
第三の人（軸）が入ると、この行きづまりに抜け道ができます。四元数の言葉でいえば、二人の平面の外へ立ち上がる直角（ℍ ＝ ℂ ⊕ ℂj）が生まれます。すると、面の中の裏返しから、外への抜け道ができます。精神分析がエディプス三角形と呼んだ三者関係への移行を、ℂ → ℍ の梯子として読む方向です。そして §3 の結合律（3人では段取りが結論を変えない）は、三者関係が持つ構造的な安定の側を写します。

**ここで、正直な留保を一つ挟んでおきます。** 「第三者が入ると閉じて安定する」は、いつも良い着地とは限りません。三者関係を早くから論じた社会学者ジンメルは、第三者の役割を3つに分けています。仲裁者だけでなく、漁夫の利を得る者、分断して統治する者もいる、というものです。数学的には、二人の積が第三の場所に「閉じる」（Q4）ことは保証されていても、その閉じ方が関係にとって良いものかどうかは、また別の話です。閉じることと、良い着地であることは、同じではありません。
...
もう一つ、この研究は「場と人が互いを作り合う」ことも示します。乳児の三角形を扱う力が高いほど、その家族の同盟（場）の質も良い、という相関があります。逆に、二人が一人を責めるような連合が起きて場が壊れると、乳児の三角形を扱う力も一時的に下がります。人の能力と、場の質は、別々の変数ではありません。互いを支え合っています。§6 の「場と人（次元−1）」も、この相互構成として読めます（ただし、この実証が言えるのは「三角形は最初から一つ・二人の和には還元されない」までです。四元数の「二つの積がちょうど第三に一致する」という一意性に対応する現象は、まだ見つかっていません。構造の類似の範囲で使っています。出典は RR-017）。

**私の感想**: 最近は、2名の破壊的で不確実な創造性を考えることが増えた。**嫌だけど、悪くない**。クラインの母子未分離のような視点——そして二者の創造性は、創造の探究でいう**揺らぎと波と渦**のことだ。（2026-07-03・二者の創造性は方向確定）

（探索中の対応であり、確定した主張ではありません。判定の記録は RR-010 Q5/Q6/D1/D2）
</details>
...
<details>
<summary><strong>コラム（寄り道）— Graves と Cayley: 八元数を最初に見つけたのは誰か</strong></summary>

四元数（3人ぶんの数）の次にある八元数（7人ぶんの数）は、誰が見つけたのでしょうか（以下は Fable5 の調べです。日付の細部までは一次資料に届いていません）。

じつは最初に見つけたのは、ハミルトンの友人 John Graves でした。ハミルトンが四元数を見つけた1843年の暮れ、Graves はそれをさらに広げて、8つ組の数（八元数）を発見していました。ところが、その公表がなかなか進みませんでした。

その間に、別の数学者 Arthur Cayley が独立に同じ数を見つけ、1845年に先に発表しました。そのため、八元数は長いあいだ「ケイリー数」と呼ばれることになりました。先に見つけた人と、先に名前を残した人が、違ってしまったのです。

数学の歴史では、こういう「早く見つけたのに、公表が遅れて名前を取られる」ことがときどき起きます。誰が本当の最初かをめぐる、静かなすれ違いの一つです。（RR-014）
...
<text x="340" y="500" text-anchor="middle" font-family="sans-serif" font-size="11" fill="#5F5E5A">中心 e₇ を通るのは色分けした3本だけ（交点なので6本に見えますが、線としては3本）</text>
</svg>
...
これは、この読み物の背骨とも響きます。§8 で「数字が合うだけの数秘術ではないか」という疑問に答えていますが、ファノ平面の構造は、現実の通信機の中で本当に仕事をしています。数学の形が現実で働いている、たしかな一例です。（RR-014）
...
<details>
<summary><strong>コラム（寄り道）— 168 という数と、クライン四次曲線</strong></summary>

上で、7人の網（ファノ平面）には対称変換がちょうど168通りある、と書きました。じつはこの168という数は、まったく別の場所にも顔を出します（以下は Fable5 の調べです）。

19世紀の数学者フェリックス・クラインが調べた「クライン四次曲線」という曲がった面があります。この面は、とても対称性が高いことで知られています。その対称性の数を数えると、ちょうど168通りになります。ファノ平面の168と、同じ数です。

これは偶然の一致ではありません。二つは、数学の奥のほうで同じ仕組み（同じ群、と呼ばれるもの）を共有しています。7という数のまわりには、こうして別々の分野の美しい構造が集まってきます。7が特別だという直観の、数学の側からの小さな傍証の一つです。（RR-014）
...
§3 で「割り算ができる」を、ざっくり「結果から巻き戻せること」と説明しました。じつは、この言葉には四つの層があります（以下は Fable5 の整理です。機械検証済み・RR-018）。最初は「割り算の本質は四つある」と書こうとしましたが、検査すると重なりや抜けがありました。そこで、性質・ノルム・過程・場所という、別々の層として並べ直しました。
...
- **B. ノルム**——掛け算をしても、各人の「大きさ」は変わりません（`|x·y| ＝ |x|·|y|`。ノルム保存）。ここは、A5=Q8 の判定済みの読み（下の 📖）につながる大事な層です。
...
> 📖 **ノルム保存を、二段で読む**（A5＝Q8・採用 2026-07-04） — 上の B（ノルム保存）を、人間関係の言葉で読むと、二段になります。
>
...
>
> だから「掛け算で成果が跳ね上がる」という手応えは、正確には「回転が位相を揃え、重ね合わせ（足し算）が増幅した」ということです（この誤解の根は §8 でさらに掘ります）。

> 📖 **単位球は、条件ではなく舞台です**（RR-018） — 上の D で「大きさ1のところ＝回し手の家」と書きました。これは「割り算ができる条件」ではありません。**舞台**です。この読み物が「回転の掛け算」と言った時点で、暗黙のうちに、大きさ1の世界（痩せも膨れもしない帯）を舞台に選んでいました。それを、ここで一度、はっきりさせておきます。
...
**もしも人間関係でいうなら**、こうです。どちらも一人では元気なのに、組み合わさると何も生まれず、しかもほどけなくなる関係——そう読むのが、本仮説の賭けです（この対応には、まだ実証がありません。検証の現在地は §7 です）。
...
量子力学の時間発展（波動関数のユニタリ発展）も、同じ性質を持ちます。位相が回り続けるだけで、情報は決して消えません（可逆）。§3 で見た「×0 だけは取り消せない」という事故、つまり情報が消えて戻せない事故が構造的に起きない——この「割れる」の意味は、「巻き戻せる回転」という見方で一つにつながります。

ここで、先回りの正確さを一つ、加えておきます。「割り戻せる」と「情報が消えない」は、重なりますが、同じではありません。「0 でなければ必ず割り戻せる」（逆元がある）は、数の世界の全域で成り立ちます。一方「回し続けても情報が消えない」が成り立つのは、大きさを保つ回転（ノルム1の掛け算）に限ります。大きさを変える掛け算は、一回ごとには戻せても、繰り返せば区別が潰れていきます（機械検証済み・RR-013a）。この読み物で「巻き戻せる関わり」と言うときは、前者（割れる）を土台に、後者（回転）へ賭けています。
...
（探索中の対応です。判定の記録は RR-010 Q7、可逆性の二層分解は RR-013）
</details>
...
「数の掛け算の話」だと思っていた問いが、じつは「球の形の話」と同じだった——分野をまたいで答えが見つかった、美しい例です。この読み物が §4 で「つむじ」の絵を使うのも、この深いつながりがあるからです。（RR-014）
...
ここで一つ、正直な注意をしておきます。これは「三つで数が一致した、だから正しい」という話ではありません（数の一致は、後から数え方を変えればどうにでもなります）。言いたいのは、**「放出／受信／あいだ」という問いの分かれ方の型が、遠く離れた別の問いでも繰り返し現れる**、ということだけです。感情の正体がまだよくわからないのと同じように、光の正体も歴史のなかで何度も見方が変わってきました。よくわからないものを考えるとき、人はこの三つの方向に分かれやすいのかもしれません。（RR-022）

</details>

ここで、体験A（§0 の、体を流れる二つの信号、生存-信頼軸の側）に戻ります。物質（生存）の軸と精神（信頼）の軸が直交するなら、意識には複素数の世界が数学的に馴染みます。
...
既存の **CN-009**（切り口と相補的射影）は、物質的/精神的な2つの切り口を「直交軸ではなく相補的射影（非可換な観測量に近い）」としていて、四象限の図式を棄却しています（📖 四象限とは、2つの軸を格子に組み、人や現象を4つの区画に振り分ける図式です。CN-009 の棄却理由は、片方を鋭くすると片方がぼやける、2軸を同時に鋭くする格子に組むと現象が痩せる、というものです）。これは、本仮説の前提①「物質⊥精神→ℂ」と正面から緊張します。
...
この節の主張を感情心理学の実証に照らして検査した結果（何が支持され、何が空振りだったか）は、§7（結果4）に畳んで置きました。

> 📖 **先回りの釘——「実部」「虚部」の意味は、今も一つに決まっていません**（C16・RQT の釘） — 物理学でも、「複素数が本当に必要か」「実部と虚部は何を意味するか」は、今も決着していません（RQT＝Relational Quantum Theory をめぐる論争など）。ですから、この読み物でも、実部・虚部が「本当は何なのか」を、一つの意味に固定しません。「物質」「精神」という当てはめは、あくまで探究中の読みの一つとして扱います。

<details>
<summary><strong>コラム（仮の説明 — 後から修正する予定）— 四元数を分解すると見えるもの: 2枚の平面・場を汚さない作用・三つの測り方・二周で閉じる</strong></summary>

このコラムは仮の説明です。ここで扱う四つは、たとえに寄せすぎると誤解を生みます。かといって厳密に書くと読めなくなります。その無理を認めて、いまは各項目を「厳密には」と「たとえると」の二層で並べて置きます。検証と対話が進むたびに、書き直します（判定の記録は RR-010 Q9a–Q9d、解説素材と機械検証は RR-011）。
...
- **たとえると**: 二人の平面に対して、第三の人は「面の外への直角」を持ち込みます。面の中でいくら回っても出られない反転（§3 コラム「二者の世界」）に、外への抜け道ができます。
...
- **たとえると**: 三人の軸は、「スピンを測る3つの直交する向き」とも読めます。測る順序を替えると結果が変わる、非可換な「切り口」3本です。切り口を非可換な観測として捉える見方（§7 結果4の「地図と色眼鏡」）と、ここで合流します。
...
（いずれも仮の説明であり、読みの採否は確定していません。後から修正します。判定の記録は RR-010 Q9a–Q9d、解説素材と機械検証は RR-011 / RR-011a）
</details>
...
まとめると、立体の感情モデルは実在する研究です。分裂から統合への成長というイメージも、参考になる型です。ただし、この二つと、この読み物の「感情という放出エネルギー」の仮説は、異なる層の話として、混同せずに書き分けておきます。（RR-020）
...
この読み物の背骨を、定理の形で一枚に固めておきます。

> **背骨の定理**
>
> 掛け算と割り算がともに成立する数の体系は、この宇宙に4つしかありません。
> 実数（1次元）・複素数（2次元）・四元数（4次元）・八元数（8次元）です。
> ——フロベニウスの定理、フルヴィッツの定理。
> （この「掛け算と割り算がともに成立する数の体系」を、数学では可除代数と呼びます。この読み物のタイトルの語です）
>
> そして4つとも、実数の軸は1本だけです。
> 実数を、全員が共有するリアルな場（出来事・現実そのもの）とするなら、登場人物は「次元 − 1」になります。
>
> - 1次元 ＝ 場だけ。誰もいません
> - 2次元 ＝ 場と、ひとり
> - 4次元 ＝ 場と、3人
...
>
> 場は増えません。いつも1本です。
> 増えるのは、場を取り巻くエネルギーです。揺らぎや振幅のような、もう一つの回転です。
> 5人の数も、6人の数も、誰にも作れません。だから 3 と 7 になります。

> 📖 **「共有する」の正確さと、あえて残す曖昧さ**（判定 2026-07-04）
>
> この「全員が共有する」で、厳密に言い切れるのは一つだけです。共有されているのは不変量（どの担い手から測っても変わらないもの）だ、ということです。各人の値が一致することではありません。深層心理の一次資料（Ogden）が言うとおり、第三は共同で作られますが、その経験は参加者ごとに同一ではありません。数学の側では、不変量は「どんな立場の入れ替えでも動かない実軸」として現れます（機械検証済み）。
>
...
>
> 探究が進むたびに、ここには新しい仮説・補足・比喩が増えます（Fable5 の調査記録: RR-015/016/018/019）。

<details>
<summary><strong>コラム（寄り道）— みんなが同じものを見られるのはなぜか: Quantum Darwinism</strong></summary>

すぐ上で、「全員が共有しているのは不変量（どの立場から測っても変わらないもの）だ」という読みを紹介しました。じつは物理にも、これを支えてくれそうな研究があります（以下は Fable5 の調べです）。

量子の世界では、ふつう、観測すると状態が変わってしまいます。それなのに、私たちの日常では、みんなが同じ月を見て、同じ机を見ています。なぜ「同じものをみんなが見られる」のでしょうか。

これに答えようとする研究プログラムが、Quantum Darwinism（量子ダーウィニズム、Zurek 2009 ら）です。考え方はこうです。ある物の情報は、まわりの環境（たくさんの光子など）に何重にもコピーされて広がっています。だから、大勢の観測者がそれぞれ別のコピーを読んでも、みんな同じ答えにたどり着きます。何重にもコピーされて生き残った情報だけが「客観的な現実」として共有される、という見方です。光子や特殊なセンサーを使った実験でも確かめられています。

これは、この読み物の「共有されるのは不変量」という読みの、物理側の支持材料になります。ただし正直に書くと、このコピーの仕組みは、対象が大きすぎたり時間が経ちすぎたりすると崩れることもあります。「場はいつでも必ず保たれる」わけではありません。（RR-016）
...
この仮説がその後の検証でどうなったか、何が支持され、何が棄却されたか、は §7 に正直に記しました。検証一巡を受けた改訂版（仮説 v2・仮）も、§7 の末尾にあります。

### 関連する探究（この読み物の土壌）

この読み物は、続けてきた探究たちの交差点に立っています。

- **[意識とは — awareness-space](https://uminomae.github.io/awareness-space/)**: 生存-信頼軸の仮説で「意識とは何か」を深掘りする場所です。本読み物の出発点（体験A の二軸）と、§5 の「意識の側の橋」は、ここにつながります。
...
正直に書いておきます。これは**まだ確立した理論ではなく、探索中の研究です**。多くの物理学者が有望だと考えていますが、標準的な素粒子の教科書に載っているわけではありません。ここで紹介するのは「同じ 8 という数が、遠く離れた分野でも顔を出している」という事実までです。素粒子論の正解として受け取らないでください。（RR-014）
...
ここまでが、一つの調査の記録です。§1 の問い——四元数の本質と、3人の関係の本質は、同じ形をしているのではないか。だとすれば、それはどんな関係のことか——に対して、現時点で何がわかり、何が外れ、何が残っているのかをまとめます。検証が進むたびに、この節を更新します。うまくいかなかった部分も含めて書きます。
...
この節には「棄却」という言葉が出てきますが、数学が死んだのではありません。この仮説は、強さの違う三つの読み方の積み重ねでできています。死んだのは、一番上の一枚だけです。
...
そして、出発点の素朴体験は、むしろ実証に支持されています。「3がベスト」は 3–4人 > 5–6人 のデータと合います。「7が上限」は上限帯 7–9 として残ります。外れたのは、「8で急に重くなる」の「急に」だけです。
...
強い読みがもっとも大胆な予言をしているのは、3と7のあいだの数です。「4,5,6人のチームは零因子で崩れ、解決ができないはず」という予言です。だから、そこが検証地点になります（5 や 6 が「新しい特別な数」として浮上したのではありません。反例の証人として呼ばれただけです）。実際のチーム研究を調べた結果、記述法則としての強い読みは棄却されました。
...
- 「理想形以外は崩落する」と読めば誤りです。これが、棄却された強い読みです。
...
ところが、棄却作業の副産物として、仮説の検証可能な核がはっきりしました。
...
§5 の橋（意識＝複素平面）を感情心理学の実証に照らした検証の記録は、下に畳んで置きます。要点だけ言うと、平面（距離=強さ・角度=種類）は心理学の定番と合流して強く支持されました。複素数の掛け算は、感情の平面では空振りでした。そのかわり、「判断のゆらぎ（確率）」の層で本当に働いていました。

<details>
<summary><strong>詳細 — 意識＝複素平面の検証: 平面は当たり、掛け算は空振り</strong>（§5 の橋を検証した記録。仮説検証の中身に興味がある人向け）</summary>


§5 の「個の意識＝複素平面」を、感情心理学の実証に照らして検査しました（詳細は RR-004）。ここでも読み方に段差をつけます。死んだ・残ったが一枚岩ではないからです。
...
**当たった部分——平面と極座標。** 感情心理学には、circumplex（円環）モデルという40年物の定番があります（Russell 1980）。無数の感情語を統計的に並べると、2次元平面上の円環に整列します。標準的な読み方は極座標です。角度が感情の種類（喜び・恐れ・安らぎ…が円周上に並びます）、中心からの距離が強度です（中心＝中立、周縁ほど強くなります）。これは、§5 の「`|z|`=意識の強度、位相=バランス」と同じ絵です。素朴体験Aの「二軸の平面」という直観は、心理学の主流モデルに合流します。

**空振りだった部分——ℂ の掛け算。** ただし、複素数が複素数である理由は、平面ではなく掛け算にあります（§2 で見たとおり、`i` の正体は回転で、数の梯子を駆動するのは「掛けて割れる」性質です）。ところが感情科学は、感情の混合や変化を足し算（線形結合・平均）で書いています。掛け算——位相の加算・強度の乗算という回転伸縮、`i` 倍（物質を精神へ回す90度）、割り算（関係の解消）——が仕事をしている場面は、調べた範囲で一つも見つかりませんでした。
...
| 個の意識＝複素数 ℂ（掛け算まで働く） | **棄却（平面読み・F5 発動確定）→ 判断のゆらぎの層への引っ越しを採用（2026-07-04）**（合成も時間変化も足し算の世界。複素数の位相が働く場所は「判断の確率」の層に実在する。なお棄却された平面読みも、視点を変えれば真理の側面の可能性がある——として保持論点に残す） |
...
ここで釘を刺しておきます。まず、自発放出の引き金には、「真空のゆらぎ」という説明のほかに「放射反作用」という別の説明もあり、二つは表裏の関係にあります。どちらか一方が正解、とは言い切れません。さらに、真空エネルギーの本当の大きさは、物理学でもまだ確定していない大問題です（宇宙定数問題と呼ばれます）。だからこれは「ここまで解けている」という話ではなく、「感情の未知さと、真空の未知さが、似た場所で手つかずに残っている」という対応として受け取ってください。（RR-016・RR-020・RR-022）
...
一番よく調べられている光ですら、正体は一つの見方には収まりません。感情も同じで、「一つの正しい見方」を急いで決めないほうがいい——というのが、このコラムの言いたいことです。（RR-022）
...
<details>
<summary><strong>コラム（寄り道）— 四元数戦争: 美しい構造が、一度は実務に負けた話</strong></summary>

四元数は、いまでこそゲームや宇宙機で活躍していますが、じつは一度、実務の世界から追い出されかけた歴史があります（以下は Fable5 の調べです。標準的な数学史の記録によります）。

19世紀の終わりごろ、物理や工学で「向きと大きさを持つ量（ベクトル）」をどう書くかをめぐって、大きな論争が起きました。一方は、ハミルトンの四元数を使う派です。もう一方は、Gibbs と Heaviside が整えた「ベクトル解析」を使う派です。四元数派は「こちらのほうが美しく統一的だ」と主張し、ベクトル派は「こちらのほうが計算が楽で実務的だ」と主張しました。かなり激しい応酬だったと伝わっています。

決着は、実務のベクトル解析派の勝ちでした。20世紀の物理や工学の教科書は、ほとんどがベクトル解析で書かれるようになり、四元数は表舞台から姿を消しました。「美しいけれど使いにくい」として、いったん負けたのです。

ところが話は終わりません。20世紀の後半、四元数は思わぬところで復活します。立体をなめらかに回す計算（3D の回転）や、宇宙機の姿勢制御では、四元数のほうが優れていたのです（前の「ジンバルロック」のコラムを参照）。一度負けた道具が、時代が変わって、別の必要とともによみがえりました。

これは §8 の「虚数は実在しないから比喩でしょ」という誤解への、歴史からの答えでもあります。役に立つかどうかは、その時代に何が必要とされるかで変わります。四元数は「実在するか」ではなく「構造が仕事をするか」で、二度目の評価を勝ち取りました。（RR-014）
...
7. **外れたことも公開します**。検証で仮説の一部が死んだら、死んだことを目立つ場所に書きます（§7）。誤解されそうな言い回しには、読者が誤解する前に、先回りの注記を置きます（§8）。
