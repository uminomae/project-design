...
>
> **背景：なぜこれを考えたか。** 私はチームを組むたびに、3人だとよく回り、7人を超えると急に重くなる、と体で感じてきました。同じころ、自分の意識には「物質的な生存」と「精神的な信頼」という二つの手ごたえが同時に流れているとも感じていました。この二つが同じ形をしているのではないか、というのが出発点の問いです。数学では、こうした二軸は複素数、その拡張が四元数（3人ぶん）・八元数（7人ぶん）にあたり、個人は複素数、組織はその拡張として読めます。くわしくは本文（<a href="#2-回る数--虚数そして四元数">§2</a> 以降）で順に説明します。
