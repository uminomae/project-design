...
>
> **正直な注意を三つ**。第一に、この「回し方の合成」という読みは**解釈の賭け**であり、まだ検証の途中（§7 結果1）。第二に、これを観測するときは「どのペアの件も、着地する第三の軸（ホーム）が一意に決まっているか」という**粗い代理指標**に落とす（§7 結果3）——代理は意味そのものではない。第三に、「掛け算」という言葉から「足し算以上に**どんどん増える**」を想像しやすいが、回転の掛け算は各人の大きさを**保つ**（増やしも減らしもしない）。増幅と打ち消しは波の**重ね合わせ（足し算）の干渉**の仕事で、回転はその揃い方を決めるダイヤルだ（§8 よくある誤解に先回りの解説）。
...
**なぜ違うか**: スローガンとしての「経営は掛け算」が想い起こさせるのは、たしかにその成長イメージだ。だがこの読み物の数学では、回転の掛け算は各人の大きさを**保つ**（|x·y| ＝ |x|·|y|、ノルム保存——機械検証済み、RR-012）。掛け算そのものは、増やしも減らしもしない。増えたり消えたりするのは**波の重ね合わせ（足し算）の干渉**のほう——振幅の位相が揃えば増幅し、逆を向けば打ち消し合う。

**正しい捉え方**: 三層で分かれる。**掛け算（回転）は各人の大きさを保つ／足し算（重ね合わせ）が増幅と打ち消しを起こす／その揃い方を決めるダイヤルが回転**だ。たとえるなら会場の拍手——会場が鳴るのは、一人ひとりがもっと強く叩いたから（大きさ）ではなく、**タイミングが揃った**から（位相）。「回転の掛け算の経営」の中身は「大きくする操作」ではなく、**向きと位相を合わせる操作**のことだ。実際にチームで起きている手応え——打ち消しや増幅——は振幅の干渉の現象で、それを設定しているのが回転、というのがこの読み物の見立て（§3 の正直な注意・RR-012・RR-013）。
</details>
