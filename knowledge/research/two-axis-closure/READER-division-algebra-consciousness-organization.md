...
> **この読み物の二つの声**——「私」（pjdhiro）の言葉は、**背景・体験・問い、そして回答への感想（しっくり感の判定）だけ**。それ以外の本文——**数学・物理の解説、文献調査、読みの候補、機械検証**——は、私の問いに対する **AI（Claude Fable 5）の回答**だ。役割分担は一貫している: **私が問い、Fable5 が候補を答え、私が採否を判定する**（判定の記録は RR-010）。数学の解説を、私が自分の言葉として語っているのではない。なお本文の「**機械検証済み**」は、Fable5 が該当の数学的主張をプログラムで数値的に確かめたことを指す——スクリプトは保存してあり、誰でも再実行できる（所在は文末の「参照」）。
>
> **判定のことば**——読みの候補につく状態ラベルは5段階で統一する: **採用**（判定済み・本文の骨格）／**方向確定**（方向は定まったが、言葉の精度はまだ調整中）／**読み候補**（Fable5 が提案し、判定待ち）／**探索中**（調査そのものが進行中）／**棄却**（判定済み・不採用。理由も残す）。あわせて2つの言葉を先に定義しておく——「**保持論点**」は、不快さや混乱ごと**急いで解かずに残す**と決めた問いのこと。「**しっくり感**」は、私（pjdhiro）が読みの採否を決めるときの体感的な基準のことで、論証ではなく**体験と照らした納得**を指す。
...
（図の「掛け算ができる／割り算ができる」は §3・§4 で定義する意味の略記——**答えが人の軸に着地する**・**結果から巻き戻せる**、という意味であって、「2人以下では掛け算そのものが定義できない」という意味ではない）

<details>
<summary><strong>📖 補足 — 「3人・7人」は、これまで何と言われてきたか（そして、この探究はどの角度か）</strong></summary>

チームの人数については、たくさんの学説と経験則がある。まず並べておきたい（棚卸しは Fable5 の調査による。詳細と出典は RR-023）:
...
並べてみると、気づくことがある（Fable5 の調査所見）。既存の説明はどれも**足し算と組み合わせの言葉**でできている——人数が増えるとコストが「足されて」いく。順序が効く話（一度きりの決断は巻き戻せない）や、ぐるぐる回る話（多数決のジレンマ: A の次は B、B の次は C、C の次はまた A）も昔から知られているが、前者は「**巻き戻せない**」話として、後者は「**困った現象**」として扱われてきた。**順序が効くのに、それぞれの大きさは保たれ、しかも巻き戻せる**——そういう掛け算（＝回転）の目で人数を見た説明は、調べた範囲では見当たらなかった。

この読み物は、その角度から探究している。既存の説明を土台に積み上げるのでも、論破するのでもない。**まだ誰も当てていなかった一面を、試しに当ててみる**——そういう探究だ。だから、ここで挙げた学説への評価ではなく、「言われてきたこと」の記録として読んでほしい。
...
>
> **ここから §4 までは、この問いへの Fable5 の回答**。数学の説明・たとえ・読みの候補は Fable5 が作り、読みの採否だけを私が判定している。
...
> 📖 **人間関係の「掛け算」とは何か** — ここがこの読み物のいちばん深い賭けで、まだ言葉が完成していない（保持論点）。問いの形はこうだ: **四元数の本質と、3人の関係の本質は、同じ形をしているのではないか。だとすれば、それはどんな関係のことか**。いまの見当——**候補の言語化は Fable5、採否の判定は私のしっくり感**（記録は RR-010）——を、出発点の二軸に忠実なかたちで書いておく。
>
...
7人を三つ組で編んだ網（ファノ平面）の対称変換（自己同型）は**ちょうど168通り**（この数は総当たりの機械検証で確認済み。群として PGL(3,2)——PSL(2,7) と同じ群であることが知られている）。そして **どの人も構造的に同格**、区別された中心が無い。**接続のためにはリーダーは要らない**——構造が自分自身のリーダー。
...
1人を区別すると（外的な非対称：希少スキル等）、その1人を通る3本の線が、残る6人を「その人と組む3組のペア」に割ってしまう——**区別が、構造の分割を連れてくる**（§4 の📖「7人への拡張——一人が動くと、3つの『あいだ』が同時に回る」と同じ配線の裏表。機械検証済み）。
...
**壊れ方の核心＝零因子**（`a≠0, b≠0` なのに `a·b=0`）。§3 で見たとおり「×0 だけは取り消せない」——結果のゼロからは元を割り戻せない。ふつうの数ではこの事故は0を掛けたときにしか起きないが、壊れた数の世界では**ゼロでないもの同士を掛けたのにゼロになる**。つまり、誰も0を掛けていないのに「復元不能」が仕組みに埋め込まれてしまう。二つの非ゼロが結合して**消える**＝割り算＝解決ができない。降りていく代償の梯子（Cayley–Dickson）：`ℝ(順序)→ℂ(順序を失う)→ℍ(可換性を失う)→𝕆(結合性を失う)→𝕊(交代性と割り算を失う)`。（📖「交代性」＝結合律の最後の名残——同じ元が2回入る掛け算（x·(x·y) など）でだけカッコを外せる、という弱い保証。𝕆 はこれをまだ持っているが、𝕊＝16元数で失う）
...
先回りの正確さをひとつ。「割り戻せる」と「情報が消えない」は、重なるが同じではない。**0 でなければ必ず割り戻せる**（逆元がある）は数の世界の全域で成り立つ。一方「回し続けても情報が消えない」が成り立つのは、**大きさを保つ回転（ノルム1の掛け算）に限る**——大きさを変える掛け算は、一回ごとには戻せても、繰り返せば区別が潰れていく（機械検証済み・RR-013a）。この読み物で「巻き戻せる関わり」と言うときは、前者（割れる）を土台に、後者（回転）へ賭けている。

もうひとつ。量子力学のこの可逆性は、**観測されていない間**（シュレーディンガー方程式に従って位相が回り続けている間）の話だ。**測定（観測）が起きた瞬間、この巻き戻し可能性は破れる**——量子の世界にも、巻き戻せない過程（射影測定）が同居している。「量子のように巻き戻せる」は、量子の片面だけを借りた言い方だ。
...
頭の毛を全部なでつけても必ず一箇所は渦になる。地球の風は球面に沿う向きの場なので、地上には**必ず無風点**がある。「完全に梳ける＝空間全体に矛盾のない向きを一斉に敷ける＝掛け算・割り算が成立する」。これができる球は **S⁰, S¹, S³, S⁷** の四つだけ（＝次元 1,2,4,8 の数の世界＝3と7）。数え方に注意——**数の世界の次元は 1,2,4,8、球面の添字 n＝虚方向の数は 0,1,3,7** で、いつも1つずれる（実軸の分）。
...
（📖 問いにある「**意識のゲート**」＝脳に届く無数の信号のうち、どれが意識に上るかが選別される入り口、という像。この読み物では「物質の信号と精神の信号が、同じレベルでこのゲートを通る」という出発点の観察として使う。詳しくは awareness-space）

ここで、体験A——§0 の、体を流れる二つの信号、**生存-信頼軸**の側——に戻る。**物質（生存）の軸と精神（信頼）の軸が直交する**なら、意識には**複素数の世界**が数学的に馴染む。
...
既存の **CN-009**（切り口と相補的射影）は、物質的/精神的な2つの切り口を「**直交軸ではなく相補的射影（非可換な観測量に近い）**」とし四象限を棄却している（📖 四象限＝2つの軸を格子に組み、人や現象を4つの区画に振り分ける図式。CN-009 の棄却理由は「片方を鋭くすると片方がぼやける——2軸を同時に鋭くする格子に組むと、現象が痩せる」）。本仮説の前提①「物質⊥精神→ℂ」と正面から緊張する。
...
- **厳密には**: 3次元の回転は q v q⁻¹ という**挟み込み**で書く。だが挟み込みという形そのものは計算の都合で、表現に依らない中身は別にある: 状況ベクトル v（実部＝実軸方向の成分、が 0 のもの）に**片側から掛けるだけ**（qv）だと、答えに実部が生じる——**「場」（実軸）に漏れる**。挟み込みだけが実部を 0 のまま保ち、大きさも保存する（機械検証済み）。
- **たとえると**: 働きかけっぱなしの関わりは、状況を変えたつもりで、**全員が共有する場のほうに漏れ込んでいる**。場を無傷に保つ形の関わりだけが「純粋な回転」になる——それを数式に書くと、たまたま「前から掛けて、後ろで戻す」挟み込みの形になる。

**③ 三つの測り方（パウリ行列）**

- **厳密には**: `i ↔ −iσx, j ↔ −iσy, k ↔ −iσz` と置くと、四元数の掛け算の規則は、量子力学でスピンを測る**パウリ行列**（2×2 の行列3つ）とぴったり一致する（機械検証済み）。行列とは「掛け算の規則を数字の表にしたもの」で、同じ表を持つ道具は同じ機械——**四元数とパウリ行列は、同じ機械の二つの筐体**だ。ただし物理の中での役割は別物になる: パウリ行列は**測る**ための道具（観測量）、四元数の i・j・k は**回す**ための道具（変換）。ここでの一致は「同じ数学的骨格を共有している」という意味に限る。
...
私は無意識に、**「大きくしたい（物質）」と「一つのまとまりでいたい（精神）」を、両方いっぺんに最大にしようとしていた**。この問いへの **Fable5 の回答**はこうだった——**この二つを、どちらも譲らずに欲張ると、きれいに成り立つ形は 3 と 7 しか残らない**。体の二軸信号と、チームの数字は、同じ一つの構造の、二つの現れ方だった。
...
> ——フロベニウスの定理、フルヴィッツの定理。
> （この「掛け算と割り算がともに成立する数の体系」を、数学では**可除代数**と呼ぶ——この読み物のタイトルの語だ）
>
> そして4つとも、実数の軸は1本だけ。
...
>
> この「全員が共有する」で、厳密に言い切れるのは一つだけ——共有されているのは**不変量**（どの担い手から測っても変わらないもの）だ、ということ。**各人の値が一致することではない**。深層心理の一次資料（Ogden）が言うとおり、第三は共同で作られるが、その経験は参加者ごとに同一ではない。数学の側では、不変量は「どんな立場の入れ替えでも動かない実軸」として現れる（機械検証済み）。
>
> では、その不変量の**本質**は何か——ここから先は探究の途中だ。「不変のものが在る」ということ自体は、量子（観測のたびに実に落ちる値・独立な観測者の一致）でも、宇宙（誰から測っても変わらない間隔）でも、存在論でも、別々の言葉で探究されてきた。**私の感触**: 半分はわかっているが、真理の側面しか掴めていないのかもしれない。だからこの読み物では一つの言葉に決めず、たとえで囲んでおく:
>
> - **距離のたとえ**——二人が別の場所から同じ二点を測ると、座標（各自の読み）は違うのに、二点のあいだの**距離**は同じになる。共有されているのは座標ではなく距離
> - **食卓のたとえ**——同じ食卓を囲んでいても、見えている景色は一人ずつ違う。共有されているのは景色ではなく、**食卓の配置そのもの**
>
> 探究が進むたびに、ここには新しい仮説・補足・比喩が増える（Fable5 の調査記録: RR-015/016/018/019）。
...
- **[意識とは — awareness-space](https://uminomae.github.io/awareness-space/)** — 生存-信頼軸の仮説で「意識とは何か」を深掘りする場所。本読み物の出発点（体験A の二軸）と、§5 の「意識の側の橋」はここにつながる。
...
> この節の**調査・検証・文献裁定は Fable5（AI）が実施**し、読み（採否）の**判定は pjdhiro** が行っている（判定の記録は RR-010）。
...
§5 の「個の意識＝複素平面」を、感情心理学の実証に照らして検査した（詳細は RR-004）。ここでも読み方に段差をつける。死んだ・残ったが一枚岩ではないからだ。
...
**当たった部分——平面と極座標。** 感情心理学には circumplex（円環）モデルという40年物の定番がある（Russell 1980）。無数の感情語を統計的に並べると、2次元平面上の**円環**に整列する。標準的な読み方は極座標で、**角度が感情の種類**（喜び・恐れ・安らぎ…が円周上に並ぶ）、**中心からの距離が強度**（中心＝中立、周縁ほど強い）。これは §5 の「`|z|`=意識の強度、位相=バランス」と**同じ絵**だ。素朴体験Aの「二軸の平面」という直観は、心理学の主流モデルに合流する。
...
外れた部分を切り、生き残った部分を締め直して、仮説を書き直した。私（pjdhiro）はこの版を採用した——**ただし、これが最終だとは考えていない**（下の「私の感想」）。
...
> 3. **複素数の掛け算が働く場所は、感情の平面ではなく「判断のゆらぎ（確率）」の層**。「意識＝ℂ」はこの層の主張として引っ越す——人の判断が古典的な確率の足し算則を破り、位相の干渉でぴったり記述される、あの場所だ。（引っ越しは 2026-07-04 に採用が決まった）
...
> 問いは、読者から予想されるもの。**回答は Fable5**（読みの採否の判定は pjdhiro・RR-010）。
...
**なぜ違うか**: その読み方（強い読み）は、実際のチーム研究に照らして**すでに棄却済み**（§7）。5人や6人のチームは普通に存在し、普通に機能する。
...
**正しい捉え方**: この読み物の「7」は記憶容量からではなく、**掛けて割れる数の世界が 8 次元（虚方向 7 本）で終わる**という数学の分類定理から来ている。だからこそ「記憶の限界なら道具で動くはず、構造の限界なら動かないはず」という**区別可能な予測**が立つ（§7）。
</details>
...
**なぜ違うか**: その危険は、この仮説自身が**検査基準として先に立てている**。数学の構造（掛け算・割り算・編み方）が現実の側で仕事をしていない対応は「装飾＝数秘」と判定するルールを最初に決め、反証条件を5つ明文化した——F1 安定した5・6人チームは常態か／F2 道具でチームの上限が動くか／F3 7人チームに「重なり合う三人組の網」が観測されるか／F4 二軸は直交か・非可換な相補か／F5 感情の掛け算は実在するか。そして実際に、そのうち2つは**発動して仮説の一部を殺した**（F1→「3と7しかない」の棄却、F5→「感情の平面＝複素数」の棄却。F4 は部分発動ののち「見かけの対立」として解消、F2 は未発動、F3 が残る主戦場。§7）。
...
この読み物は、一人の人間と複数の AI エージェントの分業で作られている。核になる規律は7つ。特定の道具に依存しない形で書く——どの LLM でも真似できるはずだ。

1. **役割を固定する——人間は問いと判定、AI は調査と候補**。人間（pjdhiro）がやるのは、体験から問いを立てることと、AI の答えに「しっくりくるか」の採否判定を下すことだけ。数学の解説・文献調査・読みの候補づくりは全部 AI の仕事。この分担を崩さない（AI が人間の言葉を代弁しない）。
2. **問いは、記録に残った実際の言葉から立てる**。各節の「私の問い」は実際の対話記録から抽出する。捏造しない。判定（採用・却下・保留）も日付つきで台帳に記録する。
3. **専門家を並行に立てて、同じ問いを別の角度から調査させる**。一つのテーマに、分野の違う専門家役の AI（例: 経営学・心理学・神経現象学・数学）を同時に走らせ、同じ形式で答えを回収する。全員に「**反証も同格に集めよ**」と指示する。
4. **すべての主張に確度と出典レベルを付けさせる**。確立事実／比喩的解釈／推測の3段タグ。出典は実在を確認したものだけ。論文を要旨までしか読めていないなら「要旨まで」と明記させる。
5. **数学の主張はプログラムで検証する**。定理の引用で済ませず、数値で確かめられる主張は実際に計算して確認する（例: 八元数の結合律の破れ、ノルム保存、零因子の実在）。スクリプトは保存し、誰でも再実行できる。
6. **別の AI に「批判役」をやらせる**。調査役とは別に批判役を立て、4つの検査をかける——最強の反論は何か／都合の良い証拠だけ拾っていないか／証拠の範囲を超えて一般化していないか／論理の飛躍はないか。重要な判断では批判役を**2体、独立に**走らせて突き合わせる。
7. **外れたことも公開する**。検証で仮説の一部が死んだら、死んだことを目立つ場所に書く（§7）。誤解されそうな言い回しには、読者が誤解する**前に**先回りの注記を置く（§8）。

<details>
<summary><strong>もっと詳しく — 実際の道具立て（Claude Code / agent-team-workflow）</strong></summary>

- 実装は Claude Code のマルチエージェント機能。手順は「**agent-team-workflow**」というスキル（再利用可能な手順書）に固定してある: **調査（SURVEY）→ 批判レビュー（REVIEW）→ 統合（CLOSE）** が基本の3拍子で、必要に応じて計画（PLAN）・実装（EXECUTE）を挟む。
- エージェントの役割は4種: **researcher**（調査。読み取り専用）／**critic**（批判検証専任）／**planner**（計画）／**worker**（執筆・実装）。ファイルへの書き込み権限は worker だけに絞る。
- 批判役の検査項目は V1〜V6 の6本: V1 参照先は実在するか／V2 主張と根拠は一致するか／V3 最強の反論は何か／V4 都合の良い証拠だけ拾っていないか／V5 証拠の範囲を超えて一般化していないか／V6 論理の飛躍はないか。
- 調査は1本ごとに番号つきファイル（RR-001, RR-002, …）に固定する。必ず含めるのは: 問い・方法・発見（確度つき）・**限界と反例**・判定待ちの問い。判定の台帳は RR-010。
- 公開前の品質ループ: 批判役のレビュー → 機械テスト（リンク・文字化け・サイト検査）→ 修正 → 反映。この §9 自体も、この手順で書かれている。
