...
> **先に、よくある3つの疑問に答えておきます。**（急いで読む人は、まずここだけでも。詳しくは各リンク先へ）
>
...
**ここで、正直に一つ。** この梯子の段（数の世界の大きさ＝次元）は、1 → 2 → 4 → 8 と倍々に増え、そして 8 でぴたりと終わります。**なぜ倍々で、なぜ 8 で止まるのか**——その理由は、正直に言うと高校数学の外にあります。数学者たちがトポロジー（あとで出てくる「つむじ」の数学）で決着させた、深い結果です。ここは「そういう定理がある」とだけ受け取って、先へ進んで大丈夫です。この読み物で体を使って掴めるのは、各段の"性質"（掛けられる・割れる・順序が効く…）を一つずつ確かめる感触までで、"なぜ 1・2・4・8 だけなのか"の証明そのものではありません。そこは数学者にまかせる部分だ、と割り切ってください。
...
ただし、じゃんけんのたとえが表せるのは、ここまで——**「3人で閉じる」ことだけ**です。さきほどのスマホの実験で見た「順番を変えると向きが変わる（`j·i = −k`）」に当たる仕組みは、じゃんけんにはありません（グー・チョキ・パーには「逆回りだとマイナス」がない）。じゃんけんで「閉じる」を掴んだら、「向きが変わる」のほうは手元のスマホで掴む——比喩を一つで欲張らず、役割を分けて受け取ってください。
...
**日常のたとえでいうと**、7人が、3人ずつの「グループLINE」を作っている状態です。グループは全部で7つ。どの一人も、ちょうど3つのグループに入っています。そして、**どの2人を選んでも、その二人がそろって入っているグループが、ちょうど1つだけ**あります（多すぎず、少なすぎず、いつも1つ）。この「どの2人にも共通のグループがちょうど1つ」という、すきまのない重なり方が、ファノ平面の正体です。図の点が人、線がグループLINE にあたります。
...
- ✅ **触って確かめられる図（対話的な小世界）**: 読むだけでなく、その場で触って挙動を直感できる図を置きました。<a href="#2-回る数--虚数そして四元数">§2</a> の「掛けると回る（大きさは変わらない）」と「同じ2回転でも順序で行き先が変わる」、<a href="#4-なぜ-7-は特別か--割り算までできる最大の人数">§4</a> の「非ゼロ×非ゼロ＝ゼロ（零因子）は 8 を超えてから」です（JavaScript が動かない環境では、同じ内容を短い文で読めるようにしてあります）。
