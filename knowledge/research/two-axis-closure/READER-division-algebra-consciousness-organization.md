...
- メンバー一人ひとりは、それぞれ自分の軸を持っています。その軸の正体は「見方・視点」ではありません。場を取り巻くエネルギーです。揺らぎや振幅のような、もう一つの回転です。それぞれが同じ出来事を、違う向きに生存↔信頼変換して回しています（認知のスタイルの話に縮めないでください。<a href="#6-帰結--二つの体験が一本につながる">§6</a> の「場は増えない。増えるのは、場を取り巻くエネルギー」と同じ線です）。
...
> **いまの位置**: ここからは組織側（3人・7人）ではなく、意識の側の話です。「二軸→平面→複素数」という橋を架けます。この橋が架かって初めて、<a href="#6-帰結--二つの体験が一本につながる">§6</a>で二つの体験がひとつにつながります。
...
ここで、体験A（<a href="#0-私の体験ここから全部が始まった">§0</a> の、体を流れる二つの信号、生存-信頼軸の側）に戻ります。物質（生存）の軸と精神（信頼）の軸が直交するなら、意識には複素数の世界が数学的に馴染みます。
...
この橋が本当に正しいかどうかは、まだ検証の途中です（結果は <a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a> 結果4に畳んであります）。ここでは「二軸を複素平面として見てみる」という出発点だけを押さえて、次の<a href="#6-帰結--二つの体験が一本につながる">§6</a>に進みます。
...
**掛け算と割り算がともに成立する数の体系は、この宇宙に4つしかありません。** 実数（1次元）・複素数（2次元）・四元数（4次元）・八元数（8次元）です（フロベニウスの定理、フルヴィッツの定理）。この「掛け算と割り算がともに成立する数の体系」を、数学では可除代数と呼びます。この読み物のタイトルの語です。

そして4つとも、実数の軸は1本だけです。実数を、全員が共有するリアルな場（出来事・現実そのもの）とするなら、登場人物は「次元 − 1」になります。
...
場は増えません。いつも1本です。増えるのは、場を取り巻くエネルギーです。揺らぎや振幅のような、もう一つの回転です（この「エネルギー」という言葉が正確に何を指すのかは、まだ言葉を決めずに探索している途中です。ここでは比喩として受け取ってください）。5人の数も、6人の数も、誰にも作れません。だから 3 と 7 になります。

個の意識は複素数 ℂ の形をとり、組織はその上の四元数（3）と八元数（7）の形をとります。物質⊥精神という一つの平面が、他者と関わって虚方向を増やしていくと、掛け算と割り算を保ったまま到達できるのは、3 と 7 だけです。個と組織は、同一の可除代数の梯子の、一人の段（ℂ）と集団の段（ℍ・𝕆）にあたります。
