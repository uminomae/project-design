...
<details>
<summary><strong>コラム（探索中の方向）— 四元数を分解すると見えるもの: 挟んで回す・三つの測り方・一周で裏返る</strong></summary>

**挟んで回す（回転子）**。3次元の回転は、実は q v q⁻¹ という**挟み込み**で書く——前から掛けて、後ろで戻す。掛けっぱなしの一方向では、純粋な回転にならない。関わりが「働きかけて、受け取り直す」**往復**ではじめて状況を回す、という読みの種。

**三つの測り方（パウリ行列）**。四元数は 2×2 の行列——量子スピンの言葉——にそのまま分解でき、i, j, k は**スピンを測る3つの直交する向き**と同じ代数になる（機械検証済み）。すると「三人の軸」は「**三つの相補的な切り口**」——順序を替えると結果が変わる測り方——とも読める。切り口を非可換な観測として捉える見方（§7 結果4の「地図と色眼鏡」）と、ここで合流する。

**一周で裏返る（二重被覆）**。四元数の回転は 360° 回すと −1——**元に戻ったように見えて、裏返っている**。720° で初めて＋1 に戻る。「循環ではなく、戻らないスパイラル」という感覚の、数学的な対応候補。

（いずれも探索中の対応。判定の記録は RR-010 Q9a–Q9d）
</details>
