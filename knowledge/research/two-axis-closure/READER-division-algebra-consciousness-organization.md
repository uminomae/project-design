...
> 二つの素朴体験と、「生存と信頼の二軸」という意識の仮説から出発する。経営の言い回し「足し算ではなく、掛け算」を本気で問い——その掛け算を**回転の掛け算**として、数の梯子（ℝ→ℂ→ℍ→𝕆）で確かめていく読み物。「なぜ 3 と 7 だけが特別なのか」を、定理・現実のたとえ・人間関係の三層で通す。定理（硬い部分）と解釈の賭けは分けて記し、検証が進むたびに §7「この調査でわかったこと」を更新する——**うまくいかなかった部分も含めて**。図は単体表示できる自己完結 SVG。

> **この読み物の二つの声**——「私」の一人称で書かれているのは、**pjdhiro の体験・問い・直感・判定（しっくり感）**。それ以外の部分——**数学・物理の解説、文献調査、読みの候補、機械検証**——は、pjdhiro の問いに対する **AI（Claude Fable 5）の回答・調査・生成物**だ。役割分担は一貫している: **AI が候補をつくり、pjdhiro が採否を判定する**（判定の記録は RR-010）。数学の解説を pjdhiro が自分の言葉として語っているのではない。
...
この問いには、たどってきた道がある。私は「意識とは何か」を、**生存と信頼の二軸**という仮説で深掘りしてきた（姉妹探究 [意識とは — awareness-space](https://uminomae.github.io/awareness-space/)。神経現象学の知見とも衝突しない形で組んである）。意識が二軸なら、意識の状態は**平面の上の点**だ。そして平面の上でいちばん自然な動きは、足し引きではなく**回転**——次の §2 で見るとおり、平面の回転を担う数が複素数、それを立体に広げた数が四元数だ。

一方で、経営の世界には「**足し算ではなく、掛け算**」という言い回しがある。人を足すのではなく、掛け合わせる。——ダジャレではなく、この言い回しを本気で問うてみたくなった。**その「掛け算」とは、どんな掛け算なのか。もし「回転の掛け算」のことだとしたら、チームには何が見えるのか。**
...
前提に置くのは、意識の二軸（体験A）だけ。7人へは、八元数の特徴で拡張するだけ——これがこの読み物の背骨だ。
...
「回転の掛け算」を確かめるには、まず数の側で「回る」とは何かを押さえる必要がある。数には「回れる」段階がある。
...
「**二つが出会うと、ちょうど"次の一人"に着地する**」。答えが、三人の中から**はみ出さない**。

> 📖 **「掛け算ができる」とは** — 答えが、同じ世界の中にあること。
>
> 3×4＝12。整数×整数は、必ず整数。**はみ出さない**。当たり前すぎて意識しないが、これが「掛け算ができる」ということの中身だ。
>
> チームにも、同じことを求めてみる。**メンバー×メンバー ＝ メンバー**——二人の組み合わせの結果が、チームの外にはみ出さないこと。2人チームでは、A×B は A でも B でもない何かになり、**必ずはみ出す**。3人で初めて、`i·j = k` のように、**どの二人を掛けても、答えが三人の中に収まる**。じゃんけんなら「勝負の結果がいつもグー・チョキ・パーの中で決まる」こと。C は答えの受け取り係ではない——**答えが、たまたま仲間だった**ということだ。
>
> **順番は別の性質**。すぐ次に出てくるとおり `i·j = k` だが `j·i = −k`——掛ける順で答えの**向き**が変わる。「はみ出さない」と「向きが変わる」は、分けて持っておくと迷子にならない。
>
> 📖 **「割り算ができる」とは** — 結果から、掛けた相手を割り出して、戻せること。
>
> 12÷4＝3。割り勘。「3 × □ ＝ 12」の □ を特定できる。関係でいえば、もつれても解いて戻せること。
>
> 例外はひとつだけ——「**×0**」は戻せない。6×0＝0、4×0＝0。**結果のゼロを見ても、元が6だったか4だったか、もう誰にも分からない**。ふつうの数でこの事故が起きるのは、0を掛けたときだけ。だから普段は安心して掛け算し、必要なら割り算で戻れる。「割り算ができる世界」とは、**この事故が構造的に起きないと保証されている世界**のこと（詳しくは §4 末尾のコラム「つむじと零因子」で）。

そして `i·j = k` だが `j·i = −k`（逆回りはマイナス）＝**向きを持った輪**。順序が効く。

下の図。左が「2人＝はみ出す」、右が「3人＝収まる（四元数の三つ組）」。同じ三角形を**人／じゃんけん／四元数**の三通りで読める。
...
**点と線でいえば**——2人はただの1本の線。矢印が押し合うだけで、決着の落ちる先がない＝**はみ出したまま**。3人目の点が入って初めて、答えが輪の中に収まる。

**現実のたとえでいえば**——じゃんけん。グーはチョキに、チョキはパーに、パーはグーに勝つ。三すくみになって初めて、「勝ち負けの仕組み」がそれ自体で完結する。

**もしも人間関係でいうなら**——二人の言い合いは、押し合いのまま終わらない。三人目がいて初めて、「じゃあ、こうしよう」が着地する場所ができる。四元数の言葉で言い直すと: 違う軸どうしの掛け算の答えは、二人のどちらでもない「**第三の直角**」を指す——3人で初めて、その席に人が立っている（`i·j = k`）。**三人目は仲裁係として雇われたのではなく、二人の積が指していた空席だった。** だから、**掛け算ができる最小の人数**が **3**。
...
> - メンバー一人ひとりは、それぞれ**自分の軸**を持っている。その軸の正体は「見方・視点」ではない——**場を取り巻くエネルギー。揺らぎや振幅のような、もう一つの回転**だ。それぞれが同じ出来事を、違う向きに生存↔信頼変換して回している（認知のスタイルの話に縮めない。§6 の「場は増えない。増えるのは、場を取り巻くエネルギー」と同じ線）。
> - **A × B ＝ A の回し方と B の回し方の合成**。二人の回転を続けて通すと、状況は二段階回る。
...
> - **3人で掛け算ができるとは**: どの二人の回し方を合成しても、**ちょうど三人目の回し方に一致する**こと。二人の議論から生まれる第三の回転が、チームの外ではなく、**すでにそこにいる C のもの**だということ。
> - **C に何が起きるか**: 仕事が回ってくる、ではない。**A と B の関わりの帰結が、C の軸の回転として着地する**——C のエネルギーが要請され、増幅され、生きる。
> - **足し算ではなく、掛け算**: 足し算は順序を替えても同じ（3＋5 ＝ 5＋3）——頭数を寄せ集める世界。回転の合成は**順序で向きが変わる**: `i·j = k` だが `j·i = −k`。A の軸を先に通すか、B の軸を先に通すかで、生まれる回転の**向きが逆**になる。チームは頭数の足し算ではなく、**順序と向きが意味を持つ掛け算**でできている。§1 で立てた「その掛け算とは、どんな掛け算なのか」という問いは、ここに着地する——**回転の掛け算**だ。これがこの読み物の賭けの核心。
>
> **正直な注意を三つ**。第一に、この「回し方の合成」という読みは**解釈の賭け**であり、まだ検証の途中（§7 結果1）。第二に、これを観測するときは「どのペアの件も、着地する第三の軸（ホーム）が一意に決まっているか」という**粗い代理指標**に落とす（§7 結果3）——代理は意味そのものではない。第三に、「掛け算」という言葉からは「足し算以上に**どんどん増える**」を想像しやすい——そしてその手応え自体は幻ではない。ただし数学の帳簿では、回転の掛け算は各人の大きさを**保つ**。跳ね上がりの正体は波の**重ね合わせ（足し算）の干渉**——位相が揃った瞬間の増幅で、回転はその揃い方を決めるダイヤルだ（§8「よくある誤解」で、この感覚の根まで掘る）。

<details>
<summary><strong>コラム（探索中の方向）— 二者の世界と、第三の軸: 白黒思考・クライン・エディプス、そして二人の創造性（波）</strong></summary>

`i² = −1` をもう一度見る。虚軸が**1本しかない世界**（ℂ）では、回転はあっても行き先が狭い——同じ軸を通し続けると、**裏返るしかない**（＋1 → −1 → ＋1 …）。

これを人の育ちに重ねる読みを、この探索は方向として持っている。発達初期の**二者関係**（精神分析でクラインが描いた、母子が未分離の世界）では、対象は**良い⇄悪いに反転する**（白黒思考・スプリッティング）。反転を止揚する**第三の方向**が、まだ無い。

第三の人（軸）が入ると——四元数の言葉では、**二人の平面の外へ立ち上がる直角**（ℍ ＝ ℂ ⊕ ℂj）が生まれると——面の中の反転地獄に、外への抜け道ができる。精神分析が**エディプス三角形**と呼んだ三者関係への移行を、ℂ → ℍ の梯子として読む方向だ。そして §3 の結合律（3人では段取りが結論を変えない）は、三者関係が持つ**構造的な安定**の側を写す。

**ただし、二者は「だめ」ではない。** 数の言葉で細かく見ると、2名の掛け算には二つの行き先しかない。**同じ軸どうし**なら `i·i = −1`——答えは人の軸に残れず、実数（誰のものでもない、場の空気のプラスマイナス）に落ちる。**違う軸どうし**なら、答えは二人のどちらでもない「第三の直角」を指す——だが2名の世界には、その置き場がまだ無い。だから2名の掛け算は、場の空気に沈むか、宙に浮くかのどちらか——**揺らぐ（波）**。

この揺らぎは欠陥ではなく、**破壊的で不確実な創造性**の相だ。姉妹探究（[創造とは — creation-space](https://uminomae.github.io/creation-space/)）が創造のプロセスに見ている像と重ねると: 2名の開いた往復＝**波・揺らぎ**、3人目が立つ瞬間＝**縁**（接続——平面の外への直角）、着地・止揚＝**渦**。「2名はだめ」ではなく、**2名は波の相、3名は渦が立つ最小構造**——役割が違う。

（探索中の対応であり、確定した主張ではない。判定の記録は RR-010 Q5/Q6/D1/D2）
</details>
...
>
> つまり**線1本 ＝ §3 の三すくみ1個**。ファノ平面とは、その輪を7本、「どの2人も必ずどれか1本を共有する」ように重ね編みした**掛け算の配線図**だ。
...
当然の疑問がここで立つ。1枚（3人）で掛け算ができるなら、2枚や3枚を貼り合わせた「中くらいの織物」はなぜないのか。
...
>
> **もしも人間関係でいうなら**——ホームとは、**その二人の関わりの帰結が着地する、第三の軸の在りか**（§3 の掛け算の読み）。ホームがない＝二人の間に生まれたものを受けて回す軸が、チームの中にない。ホームが2つ＝着地先が割れて、どちらの軸で回すのか決まらない。観測するときの粗い代理は「その二人の件の持ち込み先が一意か」（§7）。
...
<text x="110" y="238" text-anchor="middle" font-family="sans-serif" font-size="12" fill="#0F6E56">✓ 完結（§3 の三すくみ）</text>
...
- では9人（12枚）は? **織物としては編める**。だが今度は、その上の**掛け算が「割れる」を保てない**（すぐ下のコラム「つむじと零因子」の壁）。「完全な織物」と「割れる掛け算」の**両方**が立つ交点は——1枚と7枚しかない。
...
<details>
<summary><strong>コラム — なぜ4人や5人の「数の世界」は無いのか: つむじと零因子</strong>（深掘り。読み飛ばしても本筋は追える）</summary>

「者」の数 p ＝虚方向の数、全体次元 d = p+1。**掛け算も割り算もできる関係構造**を持てるのは d ∈ {1,2,4,8}、すなわち p ∈ {0,1,3,7} のみ。
...
**もしも人間関係でいうなら**——どちらも一人では元気なのに、組み合わさると何も生まれず、しかもほどけなくなる関係、と読むのが本仮説の賭けだ（この対応にはまだ実証がない。検証の現在地は §7）。

<details>
<summary><strong>コラム（探索中の方向）— 割り算は「逆回転」: 量子のように巻き戻せる関わり</strong></summary>

回転の言葉で言い直すと、掛け算＝**角度の足し算**、割り算＝**角度の引き算**——つまり**逆回転**だ。「割れる世界」とは、どんな関わり（回転）も逆向きに回せば**巻き戻せる**世界のこと。

量子力学の時間発展（波動関数のユニタリ発展）も同じ性質を持つ: 位相が回り続けるだけで、**情報は決して消えない**（可逆）。§3 で見た「×0 だけは取り消せない」——情報が消えて戻せない事故——が構造的に起きない、という「割れる」の意味は、この「巻き戻せる回転」という見方で一つにつながる。

先回りの正確さをひとつ。「割り戻せる」と「情報が消えない」は、重なるが同じではない。**0 でなければ必ず割り戻せる**（逆元がある）は数の世界の全域で成り立つ。一方「回し続けても情報が消えない」が成り立つのは、**大きさを保つ回転（ノルム1の掛け算）に限る**——大きさを変える掛け算は、一回ごとには戻せても、繰り返せば区別が潰れていく（機械検証済み・RR-013）。この読み物で「巻き戻せる関わり」と言うときは、前者（割れる）を土台に、後者（回転）へ賭けている。

（探索中の対応。判定の記録は RR-010 Q7、可逆性の二層分解は RR-013）
</details>
...
## 5. 意識の側の橋 — 二軸が直交するなら複素平面

ここで、体験A——私の体を流れる二つの信号、§1 で紹介した**生存-信頼軸**の側——に戻る。**物質（生存）の軸と精神（信頼）の軸が直交する**なら、意識には**複素数の世界**が数学的に馴染む。
...
→ この節の主張を感情心理学の実証に照らして検査した結果（何が支持され何が空振りだったか）は §7（結果4）に畳んで置いた。

<details>
<summary><strong>コラム（仮の説明 — 後から修正する予定）— 四元数を分解すると見えるもの: 2枚の平面・場を汚さない作用・三つの測り方・二周で閉じる</strong></summary>

このコラムは**仮の説明**だ。ここで扱う四つは、たとえに寄せすぎると誤解を生み、厳密に書くと読めなくなる——その無理を認めて、いまは各項目を「**厳密には**」と「**たとえると**」の二層で並べて置き、検証と対話が進むたびに書き直す（判定の記録は RR-010 Q9a–Q9d、解説素材と機械検証は RR-011）。

**① 2枚の平面（ℍ ＝ ℂ ⊕ ℂj）**

- **厳密には**: 四元数の4本の軸（1, i, j, k）は、{1, i} が張る平面と {j, k} が張る平面という**直交する2枚**に分けられ、どの四元数も `(a+bi) + (c+di)·j` と一意に書ける。2枚が共有するのは原点ただ一点——私たちの3次元では、原点を通る2枚の平面は**必ず線で交わる**（床と壁は必ず「角の線」を共有する）ので、この配置は**4次元で初めて可能**になり、正確な絵は描けない。絵の代わりに、掛け算の規則（`i·j = k` など）で扱う。`j` は 1 とも `i` とも直角——**面の外へ立ち上がる直角**だ。
- **たとえると**: 二人の平面に対して、第三の人は「面の外への直角」を持ち込む。面の中でいくら回っても出られない反転（§3 コラム「二者の世界」）に、外への抜け道ができる。

**② 場を汚さない作用（回転子）**

- **厳密には**: 3次元の回転は q v q⁻¹ という**挟み込み**で書く。だが挟み込みという形そのものは計算の都合で、表現に依らない中身は別にある: 状況ベクトル v（実部 0）に**片側から掛けるだけ**（qv）だと、答えに実部が生じる——**「場」（実軸）に漏れる**。挟み込みだけが実部を 0 のまま保ち、大きさも保存する（機械検証済み）。
- **たとえると**: 働きかけっぱなしの関わりは、状況を変えたつもりで、**全員が共有する場のほうに漏れ込んでいる**。場を無傷に保つ形の関わりだけが「純粋な回転」になる——それを数式に書くと、たまたま「前から掛けて、後ろで戻す」挟み込みの形になる。

**③ 三つの測り方（パウリ行列）**

- **厳密には**: `i ↔ −iσx, j ↔ −iσy, k ↔ −iσz` と置くと、四元数の掛け算の規則は、量子力学でスピンを測る**パウリ行列**（2×2 の行列3つ）とぴったり一致する（機械検証済み）。行列とは「掛け算の規則を数字の表にしたもの」で、同じ表を持つ道具は同じ機械——**四元数とパウリ行列は、同じ機械の二つの筐体**だ。
- **たとえると**: 三人の軸は「スピンを測る**3つの直交する向き**」とも読める——測る順序を替えると結果が変わる、非可換な「切り口」3本。切り口を非可換な観測として捉える見方（§7 結果4の「地図と色眼鏡」）と、ここで合流する。

**④ 二周で閉じる（二重被覆）**

- **厳密には**: 360° 回すと、**出来事**（挟み込みで変わる層）は完全に元に戻る——「物が360°で戻らない世界」では**ない**。−1 が残るのは**担い手**（片側で変わる層）だけで、しかも単独では観測できず、比較・干渉でだけ現れる。720° でちょうど +1。ねじれの記憶は ±1 の2値で、**二周で完全に閉じ、累積しない**（機械検証済み）。
- **たとえると**: ディラックのベルトトリック。帯の先に本を挟んで **360°** 回すと、本は元の向きに戻るのに、**帯にはねじれが残る**。さらに **360°**（計720°）で、ねじれは本を回さずに解ける。一周は、閉じて見える。だが場とのつなぎには見えない裏返りが残っていて、**二周して初めて、本当に閉じる**。（以前ここに書いていた「戻らないスパイラル」は不正確なので取り下げた——ねじれは二周でちょうど閉じる）

（いずれも仮の説明であり、読みの採否は確定していない。後から修正する。判定の記録は RR-010 Q9a–Q9d、解説素材と機械検証は RR-011 / RR-011a）
</details>
...
この読み物の背骨を、定理の形で一枚に固めておく。

> **背骨の定理**
>
> **掛け算と割り算がともに成立する数の体系は、この宇宙に4つしかない。**
> 実数（1次元）・複素数（2次元）・四元数（4次元）・八元数（8次元）。
> ——フロベニウスの定理、フルヴィッツの定理。
>
> そして4つとも、実数の軸は1本だけ。
> 実数＝全員が共有する**リアルな場**（出来事・現実そのもの）とするなら、登場人物は「**次元 − 1**」。
>
> - 1次元 ＝ 場だけ。誰もいない
> - 2次元 ＝ 場と、ひとり
> - 4次元 ＝ 場と、3人
...
>
> 場は増えない。いつも1本。
> 増えるのは、**場を取り巻くエネルギー——揺らぎや振幅のような、もう一つの回転**。
> 5人の数も、6人の数も、作れない。誰にも。**だから 3 と 7。**
...
- **p=3（四元数）**：成立。可換性を代償に、二人の積が第三の仲間に収まる**最小の集団**。
- **p=7（八元数）**：成立。結合性を代償に、意味が保存される**最大の集団**。
- **p≥8**：割り算が死ぬ（零因子）。

**個の意識は複素数 ℂ の形、組織はその上の四元数（3）と八元数（7）の形**。物質⊥精神という一つの平面が、他者と関わって虚方向を増やしていくと、**掛け算と割り算を保ったまま到達できるのは 3 と 7 だけ**。個と組織は**同一の可除代数の梯子の、solo の段（ℂ）と collective の段（ℍ・𝕆）**。

### 仮説（前提と定理を分離）

> **可除代数‑意識/組織仮説**：意識のゲートに上る信号が **物質⊥精神の二軸**であるなら、個の意識は **複素数 ℂ** の形をとる。その意識が他者と関わり創発するとき、系は虚方向を増やすが、「掛け算も割り算もできる（結果がはみ出さず、いつでも戻せる）」ことを保つ限り、非自明に成立する集団は **3（四元数）と 7（八元数）のみ**。ゆえに個（ℂ）と組織（ℍ/𝕆）は同一の可除代数の梯子の別の段であり、**組織設計とは、個の複素的意識を、掛け算と割り算を保ったまま多体へ拡張する操作**である。

- **前提（解釈の賭け）**：物質⊥精神 ／ 意識＝複素数 ／ 価値＝関係の解決（割り算ができること）。
- **定理（硬い部分）**：Frobenius・Hurwitz・Bott–Milnor–Kervaire・Adams（Hopf不変量1）・外積の定理・ハゲ球（平行化）・Fano=Steiner S(2,3,7)。

この仮説がその後の検証でどうなったか——何が支持され、何が棄却されたか——は §7 に正直に記した。検証一巡を受けた**改訂版（仮説 v2・仮）**も §7 の末尾にある。

### 関連する探究（この読み物の土壌）

この読み物は、続けてきた探究たちの交差点に立っている。

- **[意識とは — awareness-space](https://uminomae.github.io/awareness-space/)** — 生存-信頼軸の仮説で「意識とは何か」を深掘りする場所。本読み物の出発点（体験A の二軸）と、§8 の「意識の側の橋」はここにつながる。
- **[プロジェクトデザイン論 — project-design](https://uminomae.github.io/project-design/)** — 本読み物の本体。3人・7人の組織の側（四元数・八元数）を、ここで探究している。
- **[創造とは — creation-space](https://uminomae.github.io/creation-space/)** — 創造のプロセスを30の学問領域を横断して調べている探究。本読み物はその成果を参照素材として引く。

---

## 7. この調査でわかったこと — まとめ（2026-07-03 更新）

ここまでが、一つの調査の記録だ。§1 の問い——**四元数の本質と、3人の関係の本質は、同じ形をしているのではないか。だとすれば、それはどんな関係のことか**——に対して、現時点で何がわかり、何が外れ、何が残っているのかをまとめる。検証が進むたびに、この節を更新する——**うまくいかなかった部分も含めて**。
...
### 結果2 — ただし「3と7しかない」という強い読みは、現実で外れた
...
つまりここまでで説明は**二つ**ある。前章までの**可除代数**（3 と 7 だけが構造的に特別）と、この章の**コストの法則**（どの人数も特別ではなく、コストが滑らかに増えるだけ）。面白いことに、**3人ではこの二つは同じ答えを出す**（ペア数 3 ＝ 四元数の虚単位 3）。分かれるのは **7** だ——その話は、すぐ後の**結果3**で。
...
5〜6人チームは「モルタル（余分な調整コスト）で立っているアーチ」であり、3 と 7 は「構造だけで掛け算と割り算が立つ形」。だから 5 人チームが普通に働いている光景は数学への反証ではなく、**現実のチームがモルタルを使えることの証拠**にすぎない。問いは「立つか崩れるか」から「**どれだけのモルタルで立っているか**」に変わる。

### 結果3 — 生き残った牙: 7人は「三つ組の網」という予測
...
### 結果4 — 意識の側: 平面は当たり、掛け算は「判断の層」にいた

§5 の橋（意識＝複素平面）を感情心理学の実証に照らした検証の記録は、下に畳んで置く。要点だけ言えば——**平面（距離=強さ・角度=種類）は心理学の定番と合流して強く支持され、複素数の掛け算は感情の平面では空振りし、そのかわり「判断のゆらぎ（確率）」の層で本当に働いていた**。

<details>
<summary><strong>詳細 — 意識＝複素平面の検証: 平面は当たり、掛け算は空振り</strong>（§5 の橋を検証した記録。仮説検証の中身に興味がある人向け）</summary>
...
> 4. **価値＝解決（割り算ができること）**は後づけを禁じる。「どちらも単独では健全なのに、組むと吸い込み状態に入り、解体以外に出口がない」対（解決不能アトラクタ対）を**先に**定義しておき、それを零因子の対応物として使う。
> 5. **組織側は変わらない**: 3と7は「選べば、構造だけで掛け算と割り算が保てる」設計原理。7人＝重なり合う7つの三人組という予測が主戦場（合格ライン付き）。

v1 から変わったのは2点——「平面が複素数」から「観測・判断の層が複素的」への引っ越しと、「解決（割り算）」の測り方の事前固定。変わらないのは、数学の定理（無傷）と、出発点の二つの体験（無傷）だ。
...
**正しい捉え方**: 残っている主張は**設計原理**——「3と7を**選べば**、余分な調整コスト（モルタル）なしに、構造だけで掛け算と割り算を保ったまま拡張できる」。5人チームはモルタルで立っているアーチであって、崩れてはいない（§7 アーチとモルタル）。
</details>

<details>
<summary><strong>「掛け算の経営って、掛け合わせれば成果が何倍にも増えるって話でしょ？」</strong></summary>

**誤解**: 「足し算ではなく掛け算」とは、人を掛け合わせると足し算以上に成果が**増える**（1＋1 が 3 にも 4 にもなる）という主張だ。

**なぜ違うか——でも、その感覚は幻ではない**: 数学の帳簿では、回転の掛け算は各人の大きさを**保つ**（|x·y| ＝ |x|·|y|、ノルム保存——機械検証済み、RR-012）。掛け算そのものは、増やしも減らしもしない。……ここで話が終わりなら「掛け算の経営」は看板倒れだ。だが、あの「**掛け合わせた瞬間に跳ね上がる**」という手応え自体は、実在する。回転の言葉には**位相**があり、位相のあるところには**波**があり、波が重なるところには**干渉**がある——振幅の位相が揃った瞬間、重ね合わせ（足し算）は足し算以上に**増幅**し、逆を向けば打ち消し合ってゼロにもなる。私たちが昔から「掛け算みたいだ」と感じてきたのは、たぶん**この干渉の増幅**だ。感じ取っていた現象は本物で、**住所が違っただけ**——それは掛け算の帳簿ではなく、波の重ね合わせの帳簿に載っていた。

**正しい捉え方**: 三層で分かれる。**掛け算（回転）は各人の大きさを保つ／足し算（重ね合わせ）が増幅と打ち消しを起こす／その揃い方を決めるダイヤルが回転**だ。たとえるなら会場の拍手——会場が鳴るのは、一人ひとりがもっと強く叩いたから（大きさ）ではなく、**タイミングが揃った**から（位相）。「回転の掛け算の経営」とは、大きくする操作ではなく、**向きと位相を合わせる操作**。あなたが「掛け算」と呼んできたあの跳ね上がりは——**回転が位相を揃えたときに、重ね合わせが鳴る音**だ（§3 の正直な注意・RR-012・RR-013）。
</details>
...
**なぜ違うか**: その危険は、この仮説自身が**検査基準として先に立てている**。数学の構造（掛け算・割り算・編み方）が現実の側で仕事をしていない対応は「装飾＝数秘」と判定するルールを最初に決め、反証条件を5つ明文化した。そして実際に、そのうち2つは**発動して仮説の一部を殺した**（「3と7しかない」の棄却、「感情の平面＝複素数」の棄却。§7）。
...
**正しい捉え方**: この読み物が検証しているのは「掛けて割れるという構造が、人間の関係の側でも仕事をするか」。仕事をする場所（3人の輪・三人組の網）と、しない場所（感情の平面の掛け算）を、実証で選り分けている——比喩で終わらせないための検証プログラムが §7 だ。
</details>
