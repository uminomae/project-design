...
**ここから先は、ずっとこの問い「なぜ3人と7人なのか」に戻ってくる話です。** 次の <a href="#2-回る数--虚数そして四元数">§2</a>〜<a href="#4-なぜ-7-は特別か--割り算までできる最大の人数">§4</a> で「回る数」の仕組みを見ていきます。
...
- メンバー一人ひとりは、それぞれ自分の軸を持っています。その軸の正体は「見方・視点」ではありません。場を取り巻くエネルギーです。揺らぎや振幅のような、もう一つの回転です。それぞれが同じ出来事を、違う向きに生存↔信頼変換して回しています（認知のスタイルの話に縮めないでください。<a href="#なぜ実部を人数に数えないのか">§5 末尾「なぜ、実部を人数に数えないのか」</a>の「場は増えない。増えるのは、場を取り巻く虚方向」と同じ線です）。
...
**もしも人間関係でいうなら**、7人のチームでは、どの二人にも「その二人の関係が帰属する三人組」がちょうど一つあります。そして誰もが3つの三人組をかけ持ちして、重なり合う網になります。
...
> 📖 **ノルム保存を、二段で読む**（採用 2026-07-04） — 上の B（ノルム保存）を、人間関係の言葉で読むと、二段になります。
>
> 1. **各人の大きさは、関わり（回転）で保たれます**。掛け算そのものは、誰かを大きくも小さくもしません。
> 2. **全体の高まりは、重ね合わせの位相が揃うかどうかで決まります**。みんなの波の山と山が揃えば、全体が大きく鳴ります。
> 3. **その揃え方を設定するのが、回転です**。回転は、増幅の量そのものではなく、増幅を起こす「位相のダイヤル」です。
>
> だから「掛け算で成果が跳ね上がる」という手応えは、正確には「回転が位相を揃え、重ね合わせ（足し算）が増幅した」ということです（この誤解の根は <a href="#8-よくある誤解">§8</a> でさらに掘ります）。
...
> **いまの位置**: ここは短い節です。この読み物での意識の話は、背景の仮説が一つあるだけです——「意識は二軸のベクトルかもしれない」。それを置いたら、この節の後半で「なぜ実部（実軸）を人数に数えないのか」に答えて、<a href="#6-帰結--二つの体験が一本につながる">§6</a> の帰結へ進みます。
...
この読み物が意識について置く仮説は、一つだけです。体験A（<a href="#0-私の体験ここから全部が始まった">§0</a>）の二つの信号——物質（生存）と精神（信頼）——が互いに直角な二軸だとすると、**意識の状態は平面上の点、つまり二軸のベクトルとして書けるかもしれない**、というものです。平面の点は複素数として扱えます（<a href="#2-回る数--虚数そして四元数">§2</a>）。つまり意識は、数の梯子のいちばん下の段（ℂ・虚方向1本＝「ひとり」）に対応するかもしれない、ということです。

意識の話は、ここまでです。この橋が本当に正しいかは検証の途中で（結果は <a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a> 結果4に畳んであります）、この読み物では深入りせず、背景の仮説として置くだけにします。式の形や限界は、下の畳みにあります。

<details>
<summary><strong>詳しく — 橋の中身（z ＝ 物質 + i·精神）と、その限界</strong></summary>
...
<details>
<summary><strong>詳しく — 意識のモデルをめぐる中心的な緊張（直交か、非可換な相補か）</strong></summary>
...
この節の主張を感情心理学の実証に照らして検査した結果（何が支持され、何が空振りだったか）は、<a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a>（結果4）に畳んで置きました。
...
> **いまの位置**: <a href="#3-なぜ-3-は特別か--掛け算ができる最小の人数">§3</a>〜<a href="#4-なぜ-7-は特別か--割り算までできる最大の人数">§4</a> では、人を「虚方向の軸1本」と数えました（3人＝四元数の虚方向3本、7人＝八元数の虚方向7本）。でも、どの体系にも実数の軸が1本あります。帰結（<a href="#6-帰結--二つの体験が一本につながる">§6</a>）へ進む前に、「なぜ実軸（実部）は人に数えないのか」に答えておきます。ここが、この読み物の数え方の要です。
...
> **いまの位置**: ここで、<a href="#0-私の体験ここから全部が始まった">§0</a>の二つの体験（体の二軸信号と、チームの3人・7人）が、一本の話につながります。
...
**掛け算と割り算がともに成立する数の体系は、この宇宙に4つしかありません。** 実数（1次元）・複素数（2次元）・四元数（4次元）・八元数（8次元）です（フロベニウスの定理、フルヴィッツの定理）。この「掛け算と割り算がともに成立する数の体系」を、数学では可除代数と呼びます。この読み物のタイトルの語です。そして、どの体系も「実軸1本（＝全員が共有する場）＋虚方向（＝人）」でできていて、人数は次元 − 1 で数えるのでした（数え方の理由は <a href="#なぜ実部を人数に数えないのか">§5 末尾</a>で説明しました）。
...
整理すると、三つの壁を重ねたとき、「者」の数 p に許されるのは p ∈ {0,1,3,7} だけです。

- **p=0（実数）**: 関係が存在しません。
- **p=1（複素数）**: 可換で第三者が生まれず、三つ組が作れません。「一つの意識」です。
- **p=2,4,5,6**: 可除代数が存在せず、零因子で崩れます。
- **p=3（四元数）**: 成立します。可換性を代償に、二人の積が第三の仲間に収まる、最小の集団です。
- **p=7（八元数）**: 成立します。結合性を代償に、意味が保存される、最大の集団です。
- **p≥8**: 割り算が死にます（零因子）。
...
- **[意識とは — awareness-space](https://uminomae.github.io/awareness-space/)**: 生存-信頼軸の仮説で「意識とは何か」を深掘りする場所です。本読み物の出発点（体験A の二軸）と、<a href="#5-意識の側の橋--二軸が直交するなら複素平面">§5</a> の「意識の側の橋」は、ここにつながります。
...
<a href="#5-意識の側の橋--二軸が直交するなら複素平面">§5</a> の橋（意識＝複素平面）を感情心理学の実証に照らした検証の記録です。要点だけ言うと、平面（距離=強さ・角度=種類）は心理学の定番と合流して強く支持されました。複素数の掛け算は、感情の平面では空振りでした。そのかわり、「判断のゆらぎ（確率）」の層で本当に働いていました。

<a href="#5-意識の側の橋--二軸が直交するなら複素平面">§5</a> の「個の意識＝複素平面」を、感情心理学の実証に照らして検査しました。ここでも読み方に段差をつけます。死んだ・残ったが一枚岩ではないからです。
...
**当たった部分——平面と極座標。** 感情心理学には、circumplex（円環）モデルという40年物の定番があります（Russell 1980）。無数の感情語を統計的に並べると、2次元平面上の円環に整列します。標準的な読み方は極座標です。角度が感情の種類（喜び・恐れ・安らぎ…が円周上に並びます）、中心からの距離が強度です（中心＝中立、周縁ほど強くなります）。これは、<a href="#5-意識の側の橋--二軸が直交するなら複素平面">§5</a> の「`|z|`=意識の強度、位相=バランス」と同じ絵です。素朴体験Aの「二軸の平面」という直観は、心理学の主流モデルに合流します。
...
| §5 感情はどこから来るか・光の三説 | RR-022 |
| §5 感情の立体モデル・分裂から統合への成長 | RR-020 |
