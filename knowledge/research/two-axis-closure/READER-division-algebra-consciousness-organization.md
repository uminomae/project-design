...
この読み物は、二つの素朴な体験と、「生存と信頼の二軸」という意識の仮説から出発します。そのうえで、経営でよく言われる「足し算ではなく、掛け算」という言い回しを、まじめに問い直します。その「掛け算」を**回転の掛け算**として受け取り、数の梯子（ℝ→ℂ→ℍ→𝕆）をたどって確かめていきます。中心の問いは「なぜ 3 と 7 だけが特別なのか」です。図は、それだけ取り出しても意味が通る自己完結の SVG です。

<details>
<summary><strong>この読み物の読み方（二つの声・判定のことば・詳しい構成）</strong></summary>

**この読み物には二つの声があります**。一つは「私」（pjdhiro）の声です。「私」が語るのは、**背景・体験・問い、そして回答への感想（しっくりくるかどうかの判定）だけ**です。それ以外の本文——**数学や物理の解説、文献調査、読みの候補、機械での検証**——は、私の問いに対する **AI（Claude Fable 5）の回答**です。役割分担は最初から最後まで同じです。**私が問い、Fable5 が候補を答え、私が採用するかどうかを判定します**（判定の記録は RR-010 にあります）。数学の解説を、私が自分の言葉として語っているわけではありません。本文で「**機械検証済み**」と書いてあるところは、Fable5 がその数学的な主張をプログラムで数値的に確かめた、という意味です。スクリプトは保存してあり、誰でも再実行できます（場所は文末の「参照」にあります）。
...
一方、経営の世界には「**足し算ではなく、掛け算**」という言い回しがあります。人を足すのではなく、掛け合わせる、という意味です。私は、これをダジャレとしてではなく、まじめに問い直してみたくなりました。**その「掛け算」とは、どんな掛け算なのか。もし「回転の掛け算」のことだとしたら、チームには何が見えるのか。**
...
**ここから先は、ずっとこの問い「なぜ3人と7人なのか」に戻ってくる話です。** 流れは、背景と問い（<a href="#0-私の体験ここから全部が始まった">§0</a>〜ここ）→ コアの2点（<a href="#2-回る数--虚数そして四元数">§2</a>〜<a href="#4-なぜ-7-は特別か--割り算までできる最大の人数">§4</a> で「回転の掛け算と割り算で考える」、<a href="#5-なぜ実部を人数に数えないのか--場と人">§5</a> で「実部は人数に数えない」）→ 結論（<a href="#6-帰結--二つの体験が一本につながる">§6</a>）です。この2点が揃うと、3と7が出てきます。
...
> **いまの位置**: ここからは、中心の問い「なぜ3人と7人なのか」に答えるための下ごしらえです。まず「回る数」の仕組みを一段ずつ確認します。
...
- **2次元（複素数 ℂ）**：時計の針のように、なめらかに回れます。虚数 `i` の正体は「90度の回転」です。2回まわすと180度になります。つまり真後ろの `−1` になります（これが `i² = −1` の意味です）。
- **3次元を回す（四元数 ℍ）**：立体をなめらかに回すには、数を4つ束ねます。それが `i, j, k` です。四元数は、ゲームがキャラクターを回すのに今も使われている、本物の道具です。

この梯子を、ひとつずつ、たとえで確かめながら登っていきます。

## 3. なぜ 3 は特別か — 掛け算ができる最小の人数

> **いまの位置**: ここが中心の問いの前半です。「3人になると、なぜ掛け算が成立するのか」を見ます。

> **私の問い・仮説**: 昔からチームは3名がベストだと思う。四元数の本質と、3人の関係の本質は、同じ形をしているのではないか。だとすれば、それはどんな関係のことか。（対話ログ 2026-07-03）
>
> **Fable5 の回答**:

四元数の心臓部にあるのは、たった一つの掛け算の規則です。
...
**正直な注意を三つ、しておきます**。第一に、この「回し方の合成」という読みは解釈の賭けです。まだ検証の途中です（<a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a> 結果1）。第二に、これを観測するときは、「どのペアの件も、着地する第三の軸（ホーム）が一意に決まっているか」という粗い代理指標に落とします（<a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a> 結果3）。代理は、意味そのものではありません。第三に、「掛け算」という言葉からは「足し算以上にどんどん増える」を想像しやすいです。そして、その手応え自体は幻ではありません。ただし数学の帳簿では、回転の掛け算は各人の大きさを保ちます。跳ね上がりの正体は、波の重ね合わせ（足し算）の干渉です。位相が揃った瞬間の増幅で、回転はその揃い方を決めるダイヤルです（<a href="#8-よくある誤解">§8</a>「よくある誤解」で、この感覚の根まで掘ります）。
...
> **いまの位置**: 3人の話が終わったので、次は「なぜ7人までなのか」を見ます。3人のときの「あいだが回る」しくみを、そのまま7人に広げます。
...
> 📖 **ノルム保存を、二段で読む**（採用 2026-07-04） — 上の B（ノルム保存）を、人間関係の言葉で読むと、二段になります。各人の大きさは、関わり（回転）そのものでは変わりません。増えるのは、重ね合わせの位相が揃ったときの全体の高まりのほうです。回転は、増幅の量そのものではなく、増幅を起こす「位相のダイヤル」です。この二段を、たとえ（会場の拍手）つきで詳しく展開したものが <a href="#8-よくある誤解">§8</a>「掛け算の経営って、掛け合わせれば成果が何倍にも増えるって話でしょ？」です。
...
> **いまの位置**: この読み物のコアは2点です。1点目は、<a href="#2-回る数--虚数そして四元数">§2</a>〜<a href="#4-なぜ-7-は特別か--割り算までできる最大の人数">§4</a> の「回転の掛け算と割り算で考える」でした。2点目が、この節の「実部（実軸）は人数に数えない」です。この2点が揃うと、3と7が出てきます。§3〜§4 では人を「虚方向の軸1本」と数えました（3人＝四元数の虚方向3本、7人＝八元数の虚方向7本）。でも、どの体系にも実数の軸が1本あります。なぜ実軸は人に数えないのか——ここが、数え方の要です。
...
> **いまの位置**: コアの2点——回転の掛け算と割り算で考えること（<a href="#2-回る数--虚数そして四元数">§2</a>〜<a href="#4-なぜ-7-は特別か--割り算までできる最大の人数">§4</a>）、実部は人数に数えないこと（<a href="#5-なぜ実部を人数に数えないのか--場と人">§5</a>）——が出そろいました。ここで、<a href="#0-私の体験ここから全部が始まった">§0</a>の二つの体験（体の二軸信号と、チームの3人・7人）が、一本の話につながります。

私は無意識に、「大きくしたい（物質）」と「一つのまとまりでいたい（精神）」を、両方いっぺんに最大にしようとしていました。この問いへの Fable5 の回答は、こうでした。この二つを、どちらも譲らずに欲張ると、きれいに成り立つ形は 3 と 7 しか残りません。体の二軸信号と、チームの数字は、同じ一つの構造の、二つの現れ方でした。
...
**物理側の支持材料——みんなが同じものを見られるのはなぜか（Quantum Darwinism）**。量子の世界では、ふつう、観測すると状態が変わってしまいます。それなのに、私たちの日常では、みんなが同じ月を見て、同じ机を見ています。なぜ「同じものをみんなが見られる」のでしょうか。これに答えようとする研究プログラムが、Quantum Darwinism（量子ダーウィニズム、Zurek 2009 ら）です。考え方はこうです。ある物の情報は、まわりの環境（たくさんの光子など）に何重にもコピーされて広がっています。だから、大勢の観測者がそれぞれ別のコピーを読んでも、みんな同じ答えにたどり着きます。何重にもコピーされて生き残った情報だけが「客観的な現実」として共有される、という見方です。光子や特殊なセンサーを使った実験でも確かめられています。これは、この読み物の「共有されるのは不変量」という読みの、物理側の支持材料になります。ただし正直に書くと、このコピーの仕組みは、対象が大きすぎたり時間が経ちすぎたりすると崩れることもあります。「場はいつでも必ず保たれる」わけではありません。
...
> **いまの位置**: ここまでの数学の話（<a href="#2-回る数--虚数そして四元数">§2</a>〜<a href="#6-帰結--二つの体験が一本につながる">§6</a>）を、実際のチーム研究・感情心理学の実証データと突き合わせます。「数学的には正しいが、現実に当てはめると何が生き残り、何が見送りになったか」の記録です。
...
**続報（2026-07-03）——F5 は発動が確定しました。** 感情の混ざり方（足し算）だけでなく、時間の中での動き方を扱う標準モデルも調べました。そこにあったのは、「基準点にばねで引き戻される点＋ゆらぎ」という、やはり足し算の世界でした。回転伸縮も、90度回す `i` も、割り算も、感情科学の語彙には登場しません。だから約束どおり、正直に格下げします。感情の平面は「複素数の平面」ではなく、「平面＋極座標」です。——ところが、話はそこで終わりませんでした。電卓の掛け算ボタンが押されている部屋が、別に見つかったのです。感情の平面ではなく、判断のゆらぎを扱う部屋です。すぐ後の続報で説明する「質問の順番」の実験と、同じ場所です。人の判断の確率は、古典的な確率の足し算ルールを系統的に破ります。そのズレは、複素数の位相（角度の差）が作る「干渉」でぴったり記述されます。複素数が心理学で本当に給料をもらっている職場は、感情の平面ではなく、この確率の層でした。仮説をそちらへ引っ越しさせるかどうかは、いったん保留にしていました。その後、判定が出ています。引っ越しは採用です（2026-07-04。ただし最終ではありません。見送りにした「感情の平面＝複素数」の読みも、真理の一面である可能性としては残してあります）。
