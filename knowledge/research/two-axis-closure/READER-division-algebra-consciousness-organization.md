...
>
> **この読み物は、この問いを二つの別々の見方で追います（混ぜません）。** **<a href="#a-意識が二軸のとき--二者は四元数三者は八元数発達の見方">見方A</a>**＝意識を平面とみて、二人・三人と関わりが増えると数がどう変わるか（発達の話。二者＝四元数、三者＝八元数。クライン→エディプスと接続）。**<a href="#3-なぜ-3-は特別か--掛け算ができる最小の人数">見方B</a>**＝場を実軸・人を虚軸とみて、なぜ3人と7人か（上の答え①〜③はこちら）。同じ四元数・八元数を別の意味で使う別々の見方なので、A→B の順で分けて説明します。
...
> **この節の内容**: 中心の2点、「回転の掛け算と割り算で考える」（<a href="#2-回る数--虚数そして四元数">§2</a>〜<a href="#4-なぜ-7-は特別か--割り算までできる最大の人数">§4</a>）と「実部は人数に数えない」（<a href="#5-なぜ実部を人数に数えないのか--場と人">§5</a>）がそろいました。この2点を合わせると、<a href="#0-私の体験ここから全部が始まった">§0</a> の二つの体験（体に流れる二軸の信号と、チームの3人・7人）が一つの話としてつながります。ただし、そう見えるという帰結であって、証明ではありません。

私は無意識に、「大きくしたい（物質）」と「一つのまとまりでいたい（精神）」を、両方いっぺんに最大にしようとしていました。この問いへの Fable5 の回答は、次のとおりです。この二つをどちらも譲らずに満たそうとすると、数の上で（回転という見方で考えるかぎり）きちんと成り立つ形は 3 と 7 だけになります。そう考えると、体の二軸の信号とチームの数字は、同じ一つの構造の二つの現れ方として見えてきます。ただし、そう見えるというだけで、それが正しいと言えるわけではありません。
...
だから、個の意識は複素数 ℂ（場と、ひとり）の形をとり、組織はその上の四元数（場と、3人）と八元数（場と、7人）の形をとります。物質⊥精神という一つの平面が、他者と関わって虚方向を増やしていくと、掛け算と割り算を保ったまま到達できるのは、3 と 7 だけです。個と組織は、同一の可除代数の梯子の、一人の段（ℂ）と集団の段（ℍ・𝕆）にあたります。帰結は、これだけです。
...
> **可除代数‑意識/組織仮説**: 意識のゲートに上る信号が物質⊥精神の二軸であるなら、個の意識は複素数 ℂ の形をとります。その意識が他者と関わり創発するとき、系は虚方向を増やしますが、「掛け算も割り算もできる（結果がはみ出さず、いつでも戻せる）」ことを保つ限り、非自明に成立する集団は 3（四元数）と 7（八元数）だけです。ゆえに個（ℂ）と組織（ℍ/𝕆）は同一の可除代数の梯子の別の段であり、組織設計とは、個の複素的意識を、掛け算と割り算を保ったまま多体へ拡張する操作である、と言えます。
