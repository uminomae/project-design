...
**もしも人間関係でいうなら**——どちらも一人では元気なのに、組み合わさると何も生まれず、しかもほどけなくなる関係、と読むのが本仮説の賭けだ（この対応にはまだ実証がない。検証の現在地は §9）。
...
→ この節の主張を感情心理学の実証に照らして検査した結果（2026-07-03、何が支持され何が空振りだったか）は §9 に記す。

---

## 8. 帰結と仮説 — 二つの体験が一本につながる
...
この仮説がその後の検証でどうなったか——何が支持され、何が棄却されたか——は次の §9 に正直に記す。検証一巡を受けた**改訂版（仮説 v2・仮）**も §9 の末尾に置いた。
...
## 9. 検証の現在地 — 何が死に、何が残ったか（2026-07-03 更新）
...
### 意識＝複素平面の検証 — 平面は当たり、掛け算は空振り（2026-07-03 追記）
...
**なぜ違うか**: その読み方（強い読み）は、実際のチーム研究に照らして**すでに棄却済み**（§9）。5人や6人のチームは普通に存在し、普通に機能する。

**正しい捉え方**: 残っている主張は**設計原理**——「3と7を**選べば**、余分な調整コスト（モルタル）なしに、構造だけで閉じたまま拡張できる」。5人チームはモルタルで立っているアーチであって、崩れてはいない（§9 アーチとモルタル）。
</details>
...
**正しい捉え方**: この読み物の「7」は記憶容量からではなく、**掛けて割れる数の世界が 8 次元（虚方向 7 本）で終わる**という数学の分類定理から来ている。だからこそ「記憶の限界なら道具で動くはず、構造の限界なら動かないはず」という**区別可能な予測**が立つ（§9）。
</details>
...
**なぜ違うか**: その危険は、この仮説自身が**検査基準として先に立てている**。数学の構造（掛け算・割り算・編み方）が現実の側で仕事をしていない対応は「装飾＝数秘」と判定するルールを最初に決め、反証条件を5つ明文化した。そして実際に、そのうち2つは**発動して仮説の一部を殺した**（「3と7しかない」の棄却、「感情の平面＝複素数」の棄却。§9）。
...
**正しい捉え方**: この読み物が検証しているのは「掛けて割れるという構造が、人間の関係の側でも仕事をするか」。仕事をする場所（3人の輪・三人組の網）と、しない場所（感情の平面の掛け算）を、実証で選り分けている——比喩で終わらせないための検証プログラムが §9 だ。
</details>
