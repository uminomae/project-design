...
> 二つの素朴体験と、「生存と信頼の二軸」という意識の仮説から出発する。経営の言い回し「足し算ではなく、掛け算」を本気で問い——その掛け算を**回転の掛け算**として、数の梯子（ℝ→ℂ→ℍ→𝕆）で確かめていく読み物。「なぜ 3 と 7 だけが特別なのか」を、定理・現実のたとえ・人間関係の三層で通す。定理（硬い部分）と解釈の賭けは分けて記し、検証が進むたびに §10「検証の現在地」を更新する——**うまくいかなかった部分も含めて**。図は単体表示できる自己完結 SVG。
...
**三つの測り方（パウリ行列）**。四元数は 2×2 の行列——量子スピンの言葉——にそのまま分解でき、i, j, k は**スピンを測る3つの直交する向き**と同じ代数になる（機械検証済み）。すると「三人の軸」は「**三つの相補的な切り口**」——順序を替えると結果が変わる測り方——とも読める。切り口を非可換な観測として捉える見方（本文 §10 の「地図と色眼鏡」）と、ここで合流する。
