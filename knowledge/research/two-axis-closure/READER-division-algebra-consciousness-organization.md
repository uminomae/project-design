...
この二つの体験を、この読み物は**二つの別々の見方**で追います。**<a href="#a-意識が二軸のとき--二者は四元数三者は八元数発達の見方">見方A</a>**＝意識を平面とみて、二者・三者へと関わりを広げる発達の話（<a href="#2-回る数--虚数そして四元数">§2</a> の次の節。二者＝四元数、三者＝八元数）。**見方B**＝場を実軸・人を虚軸とみて、なぜ3人と7人かを問う話（<a href="#3-なぜ-3-は特別か--掛け算ができる最小の人数">§3</a> 以降）。この二つは、同じ四元数・八元数を別の意味で使う、別々の見方です。混ぜると混乱するので、分けて説明します。以下の折りたたみは、意識＝複素平面という橋の細部と、それへの反論・脇道です（開かなくても本文は読み進められます）。
...
この橋（意識＝複素平面）が本当に正しいかは検証の途中です（結果は <a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a> 結果4に畳んであります）。
...
この橋の主張を感情心理学の実証に照らして検査した結果（何が支持され、何が空振りだったか）は、<a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a>（結果4）に畳んで置きました。
...
> **⚠ この節は調査中（未検証）です。** 3と7（見方B）はすでに調べ終わっていますが、こちらの「意識＝二軸 → 発達」の見方は、あとから出てきた方向で、**まだ調べていません**。確かめられた結果ではなく、探索中のスケッチです。三者＝八元数という対応が妥当かどうかは Issue #124 でレビュー中で、調べながらこの節を更新します。

> **この節の内容**: ここからは、体験A（意識が二軸）のほうを追います。個人の意識を一つの平面とみて、関わる人数が二人・三人と増えると、数がどう変わるかを見ます。これは、次の <a href="#3-なぜ-3-は特別か--掛け算ができる最小の人数">§3</a> 以降で扱う「なぜ3と7か」とは**別の見方**です。同じ四元数・八元数という数を使いますが、意味づけが違います。混ざらないように、先にこちらを一つの話として完結させます。
...
- **たとえると**: 三人の軸は、「スピンを測る3つの直交する向き」とも読めます。測る順序を替えると結果が変わる、非可換な「切り口」3本です。切り口を非可換な観測として捉える見方（<a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a> 結果4の「状態と測り方の区別」）と、ここでつながります。
...
一方、もう一つの体験A（意識の二軸）を数に対応させる話は、この3と7には混ぜません。個人の意識を物質/精神の平面（複素数 ℂ）とみて、二者＝四元数、三者＝八元数と読む見方A は、まだ調べていない別の方向だからです（調査中・Issue #124）。個人の意識＝複素平面という読みが実証にどこまで耐えるかだけは、<a href="#7-この調査でわかったこと--まとめ2026-07-03-更新">§7</a> 結果4で検証しています（平面は支持され、複素数の掛け算は空振りでした）。二つの体験は、同じ一つの好奇心から出発しましたが、いまは二つの別々の見方で追っている、というのが正直な現在地です。
...
数学の側（数の梯子が1,2,4,8で止まること）は確定した定理で、揺らいでいません。ここから先の4つの結果は、「その数学的な事実を、現実のチームや意識にそのまま当てはめてよいか」を検証したものです。それぞれ、最新の結論だけをまず示し、詳しい検証の経緯は畳んであります。
...
**なぜ違うか**: この読み物が主張しているのは、「脳が量子コンピュータだ」ということでも、「人間関係が量子力学そのものだ」ということでもありません。主張しているのは、もっと控えめなことです。人の判断の確率が、量子論の測定規則と同じ「型」の振る舞いを示す、という統計的なパターンです（§7 結果4）。これは「量子認知」と呼ばれる、心理学の中の一分野で調べられていることで、脳の中で本当に量子力学の現象が起きている、という主張ではありません。この読み物では、量子の言葉が仕事をしている場所（判断の確率の層）と、していない場所（感情の平面の掛け算）を、実証で分けています。
