...
<div class="microworld glow-card" data-microworld="double-cover" role="group" aria-label="対話：720度で閉じる二重被覆">
<p class="mw-title">触ってみる — 360度では閉じない、720度で閉じる</p>
<p class="mw-fallback">（この対話図は JavaScript で動きます。動かない環境では: 担い手の符号は <code>cos(角度÷2)</code>。0度で +1、360度で −1（物は戻っているのに閉じていない）、720度でふたたび +1（本当に閉じる）。符号は ±1 の2値で、二周でちょうど閉じ、累積しません。）</p>
</div>
...
- ✅ **触って確かめられる図（対話的な小世界）**: 読むだけでなく、その場で触って挙動を直感できる図を四つ置きました。<a href="#2-回る数--虚数そして四元数">§2</a> の「掛けると回る（大きさは変わらない）」と「同じ2回転でも順序で行き先が変わる」、<a href="#4-なぜ-7-は特別か--割り算までできる最大の人数">§4</a> の「非ゼロ×非ゼロ＝ゼロ（零因子）は 8 を超えてから」、<a href="#5-なぜ実部を人数に数えないのか--場と人">§5</a> の「360°では閉じず 720°で閉じる」です（JavaScript が動かない環境では、同じ内容を短い文で読めるようにしてあります）。
- ⚠️ **読者と作り手が見方を重ねる共有の場**: これはまだ実装していません。いまの判定ループ（性質→読み候補→pjdhiro の判定→反映）はページの外で回っています。これからの宿題として、正直に残しておきます。
