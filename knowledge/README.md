...
- pjdhiro の雑感メモ・気づきのログは `notes/` に配置する（原文保存、解釈は急がない。思考・価値観の理解用）
- 外部プラットフォームの仕様・制約のうち複数 repo に効くものは `reference/` に配置する（1 repo でしか使わない知見は、その repo を正本にする）
...
| `notes/` | pjdhiro の雑感メモ・気づきのログ。思考・価値観・視点の理解用コーパス（群盲象を評す式に集積） |
| `evidence/` | 調査・探索の根拠データ |
| `meta/` | テーマ横断のメタ情報・評価メモ |
| `research/` | 外部 LLM（ChatGPT Deep Research 等）による調査レポートのハブ |
| `reference/` | 外部プラットフォームの仕様・制約（複数 repo に効くもの）。1ファイル1事実・確認日と一次ソース付き |
| `schema/` | データスキーマ定義 |
