...
## 12. 理解度チェック（読者向け・任意クイズ／コンテンツパターン）

2026-07-08 の pjdhiro との対話で確立。Litt の「解説に埋め込むクイズ」を読者向けに実装するパターン。
...
| 配置 | 正本 MD に静的マークアップ（`<div class="reader-quiz glow-card" data-quiz>` ＋ 質問・`<ol class="quiz-choices">`・`<details class="quiz-explain">`答えと解説）を書く。本文の末尾側（読み終えた人の振り返り位置）に置く |
| 実装 | `src/reader-quiz.js`（vanilla・依存なし）が選択肢をボタン化し「押すとその場で答え合わせ」を足す。インライン `<script>` 禁止。no-JS では静的リスト＋ details で同内容が読める（SEO/LLM 発見性も同時に満たす） |
| VI | §0a 準拠。パネルは `glow-card`、スタイルは `reader.css` の `.quiz-*`（tokens の上のみ）。選択肢は実体 `<button>` 44px 以上、状態は `aria-live` の `.quiz-status` |
| 文面 | 誤答への応答は誤りの指摘でなく気づきの提供（「そう読めるのも自然です」＋根にある実在現象を解説に書く）。冒頭で「関門ではない」ことと「誤読はページ側の欠陥として直す方針」を明示する |
