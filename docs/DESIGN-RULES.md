...
**背景シェーダーの負荷上限（2026-07-05 対話で確立）**: 背景はあくまで背景であり、**スクロールがカクつくほど重いシェーダーはランダム選択プールに入れない**。目安として、フルスクリーン @ devicePixelRatio 2 で毎ピクセルの反復が重いもの（体積レイマーチ×3Dノイズ、微分 FBM の多重サンプルなど）は除外する。除外したファイルは削除せず `src/shaders/` に参照用として保持し、プール（`src/content/site-data.mjs` の `SHADER_PATHS`）のコメントに除外理由を書く。適用例: main2-webgpu（12ステップ体積レイマーチ）・main3-webgpu（微分FBM二重サンプル）を除外し、main-webgpu・main4-webgpu の軽量2本のみ選択（pjdhiro 報告「特定の背景でカクカクする」を受けた措置。4本ランダム化 fd7d644 以前は main-webgpu 1本採用で問題なし）。
