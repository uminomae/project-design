...
| 2026-07-08 | READER に対話的 Micro-worlds を4図実装（複素数回転・四元数非可換・零因子・720°二重被覆。`src/reader-microworlds.js`・canvas・VI 準拠・数理は node 検証済み）。720°は「裏の針／糸の球／ベルト」の3描画も試作 | **pjdhiro 却下「これまでの4つのどれもがわかりづらい」（pd#128・全撤去）**。数理は正しくても、読み物の直感喚起には効かなかった。「触れること」自体が理解に効くとは限らない。静的図＋比喩＋読解テストの方がこの読み物では有効。再挑戦はしっくり来る表現の当てを得てから。経緯 RR-014 §G-3 |
...
（かつてここに §10「Micro-worlds パターン」を置いたが、pd#128 の対話的可視化が「わかりづらい」と却下・撤去されたため削除した。却下記録は §8b・経緯は RR-014 §G-3。読み物での対話的可視化は、しっくり来る表現の当てを得るまで採らない。）
