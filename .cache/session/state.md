...
## 🔵 CLI 作業中 2026-07-05 (seq03) — pd#115 READER 骨格再設計（#125）計画中＋見方A/B分離（develop aa83731・push 済み）
- **【手順の立て直し（pjdhiro「作業手順が大混乱。整理して。issueに記述し計画的に骨格を見直し、段落ごとに適切か1つずつ、関数モジュールのように接続の流れをチェックする工程を入れて」）】**:
  ...
  - **Phase2 配置マップを #125 に記述**（各ブロックの新セクション＋役割）。次は Phase3 実装（物理リオーダー→再採番→アンカー張替→段落サブタイトル微調整→品質ループ）
