...
- **次アクション（pjdhiro 専権）**: ①ローカルでリロードを繰り返し、カクつきが消えたか体感確認（重い2本はもう選択されない）②develop の READER/LP をレビュー → develop→main マージ（公開判断）。critic minor 残2件は次回改訂送り
- HEAD: develop=**81bc47d**（push 済み）
