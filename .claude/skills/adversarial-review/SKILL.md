...
> **⚠️ 公開ゲート用途の必須工程（cs#261 W8 の教訓）**: パネルの生出力は verdict（例:「独立でない＝弱い証拠」）である。**公開ページに載せる前に、Synthesis の後で「判定 → 学習素材への翻訳」ステップを必ず挟む**——読者にとっての意味（知の歴史・発想の射程）に書き直す。verdict をそのまま公開文書に流し込んではならない（READER の主旨は正誤判定でなく解説）。

## 7. 決定事項と残課題

- [x] 形態: **独立スキル + agent-team-workflow REVIEW「Adversarial モード」統合の両方**（pjdhiro 2026-07-10）
- [x] 正本の置き場: **pd 正本＋cs ポインタ**（共通スキル正本管理ルール準拠）
- [x] cs での試行1件: cs#261 W8（三方面収束の独立性）で完了。縮退構成でも検証可能な史実に収束し機能。**採用**（pjdhiro 2026-07-11）
- [ ] 外部 CLI（Codex）の標準パネリスト化の可否（コスト・認証の運用）— 環境に Codex 導入後に再検証
