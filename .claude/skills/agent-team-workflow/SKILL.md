...
### Adversarial モード（cs#260、alignment に `review_mode: adversarial` で有効化）

高スティクスかつ**係争的または検証可能**な判定に使用。Voting の弱点＝同一モデル critic の盲点相関（diversity illusion: 全員一致が見かけほどの証拠にならない）を、**誤りの非相関×検証優位**で突破する。手順の正本は `adversarial-review` スキル（pd `.claude/skills/adversarial-review/SKILL.md`）。

```
Main（ファシリテータ）が Round 0 トリアージ（賭け金基準）＋枠ゲート
  → パネリストを異質性最大化順に調達（別ファミリー CLI > 別 Claude モデル > 方法論分岐）
  → Round 1: blind 並列回答 → 検証ゲート（ghost panelist 排除）
  → Round 2: 相互批判（「最低1つの中心的主張に反対せよ」強制、再現による反証）
  → Round 3: 譲歩/防御＋反証条件つき確信度
Main が統合: 合意点（収束の証拠力を同族/異種で格付け）/対立点（裁定）/結論（監査証跡）
  → No averaging: 対立は保存して裁定する
```

| 項目 | Single | Voting | Adversarial |
|------|--------|--------|-------------|
| critic 数 | 1 | 2-3 | 2-4 パネリスト × 3 ラウンド |
| 非相関 | なし（自己盲点） | 弱（同一モデルの独立試行） | **強（別ファミリー/別モデル/方法論分岐）** |
| コスト | 低 | 中-高 | 高（人数×ラウンド） |
| 信頼度 | 標準 | 高（三角測量） | 最高（生成と検証の利益相反を排除） |
| 適用場面 | 通常タスク | 高信頼リサーチ、重要設計判断 | 係争的結論の裁定、公開前ゲート、アーキテクチャ選定、根本原因仮説 |

> **モード選択の目安**: 「正しいかどうかを確かめたい」→ Voting。「間違っているのに自信満々、を検出したい」→ Adversarial。同族パネルしか組めない環境では Adversarial は方法論分岐に縮退し、収束の証拠力を格下げして報告する（実測報告の原則）。
